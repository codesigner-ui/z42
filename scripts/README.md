...
| `z42 xtask.zpkg regen` | 编译器变更使 `.zbc` 基线漂移 | `dotnet` + `z42.Driver` | run-golden 按组件镜像：`src/tests/X`→`artifacts/build/tests/X`、`src/libraries/<lib>/tests/X`→`artifacts/build/libraries/<lib>/tests/X`（gitignored，不污染 src）；committed 字节基线 `src/tests/zbc-format/*/source.zbc` 就地重写 |

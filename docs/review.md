...
## C2. JIT helper `regs[i].clone()` per op ✅ P0+P1 shipped

> Status (2026-05-28): P0 (per-register `IrType` info reaches Rust JIT) shipped
> in `8ef184e6` (REGT section in `.zbc`, zbc 1.7→1.8 / zpkg 0.8→0.9 strict-pin bump).
> P1 shipped in five commits — Value enum locked at 24 B with `#[repr(C, u8)]`
> (`a41050b8`); `Add/Sub/Mul` go native `iadd/isub/imul` (`98426e40`);
> `Eq/Ne/Lt/Le/Gt/Ge/And/Or/Not` go native `icmp/band/bor/bxor` (`fc3936f0`);
> `BrCond` reads i8 payload byte directly (`3727e469`). Measured on
> M-series macOS, 10M-iter SumSquares loop: **456 ms → 302 ms = 1.51×**
> end-to-end (split: 1.16× from arith/cmp inlining, additional 1.30× from
> BrCond). Below the spec's aspirational 2-5× — remaining gap is dominated
> by `jit_check_safepoint` (helper call at every back-edge for GC
> cooperation; next obvious win). Bench scenario at `bench/scenarios/
> 04_c2_p1_arith_loop.z42`.
...
| 🟡 | **`ISymbol` 公共抽象**（F2.2）— Phase 1 done 2026-06-03 (add-isymbol-base-phase1): `ISymbol` 基接口 (`Name` / `Kind` / `DeclarationSpan` / `Visibility`) + `SymbolKind` enum + 现有 `IMemberSymbol` 升级为 `ISymbol` 子类型。`MethodSymbol` / `FieldSymbol` 加 `Kind` getter。`SemanticModel.GetSymbol(Expr) -> ISymbol?` 落地（基于 BoundCall.Symbol）—— 顺便补齐 F2.3 Phase 1 当时被阻塞的 GetSymbol 方法（3 of 5 done）。Phase 2 (INamedTypeSymbol + Z42ClassType impl) / Phase 3 (ILocalSymbol + BoundIdent 路径) / Phase 4 (OriginalDefinition / Construct generic 路径) 独立 spec。 | 6 | Phase 1 done | 跨文件符号身份；解锁 find-references / rename |
| 🟡 | **`SemanticModel` 按需 binding`**（F2.3 Phase 1）— **3/5 query 方法落地**：`GetBoundExpression` + `GetExpressionType` + `ExpressionBindings` (2026-06-03 add-semantic-model-query-api) + `GetSymbol(Expr)` (2026-06-03 add-isymbol-base-phase1，复用 F2.2 ISymbol)。剩余 `GetDeclaredSymbol(Item)` 依赖 F2.2 Phase 3 (ILocalSymbol)，`GetDiagnostics(Span)` 需把 DiagnosticBag 灌进 SemanticModel —— 独立 spec。 | 6 | Phase 1 (3/5) done | query API；IDE / analyzer 前置 |
...
| 🟡 | **Binder 层级**（F2.4）— Phase 1 done 2026-06-03 (add-binder-hierarchy-phase1): `Binder` abstract base + 3 stub 子类 (`GlobalScopeBinder` / `InMethodBinder` / `InBlockBinder`) + 完整 design doc (`docs/design/compiler/binder-hierarchy.md`，含 Phase 2-5 migration plan)。Phase 1 scaffold NOT 接入 TypeChecker —— design doc 是核心 deliverable，让未来 agents 不必从零设计就能继续 migration。Phase 2 (第一个 consumer：`BindClassMethods` 迁到 InClassBinder + InMethodBinder) / Phase 3 (Block scopes) / Phase 4 (Lambda + catch + pinned) / Phase 5 (TypeEnv 退役) 独立 spec。 | 6 | Phase 1 done | scope 语义清晰，加 L3 特性时不爆炸 |

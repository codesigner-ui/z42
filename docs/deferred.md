...
- **来源**：`docs/spec/archive/2026-05-03-add-isubscription-wrapper/` design Decision 3
...
- **来源**：`docs/spec/archive/2026-05-08-split-typechecker-calls/`
...
- **D-10**（13_assert string.Contains 误指 LinkedList.Contains）：2026-05-04 排查发现并不存在真实的 dispatch bug。
  ...
  根因是 `./scripts/test-vm.sh` 入口未强制重建 stdlib + golden，由 `docs/spec/archive/2026-05-04-fix-test-vm-stale-artifacts/` 修复。条目从此移除。

- **D-6**（嵌套 delegate dotted-path 外部引用）：2026-05-04 落地，由 `docs/spec/archive/2026-05-04-add-nested-delegate-dotted-path/` 实施。
...
- **D-7-residual**（单播 event IDisposable token + 严格 access control）：2026-05-04 落地，由 `docs/spec/archive/2026-05-04-add-singlecast-event-idisposable-token/` 实施。
...
- **D-1b**（WeakRef ISubscription wrapper）：2026-05-04 落地，由 `docs/spec/archive/2026-05-04-add-weak-ref-subscription-wrapper/` 实施。
...
- **expand-jit-type-args**（D-8b-3 Phase 2 follow-up：JIT 路径 type_args propagation）：2026-05-07 落地，由 `docs/spec/archive/2026-05-07-expand-jit-type-args/` 实施。
  扩 `jit_obj_new` 签名加 `(*const String, usize)` 末尾两参，translate.rs `Instruction::ObjNew` 把 IR `Vec<String>` 直接当 raw pointer 传（IR 存储跨 module 生命周期有效）。helper 在 alloc 后 ctor 前 populate `ScriptObject.type_args`，与 interp 路径同 atomic 顺序。新增 0 测试 / 移除 5 处 `interp_only` 标记（multicast_func_aggregate / multicast_predicate_aggregate / default_generic_param / default_generic_param_pair / default_generic_param_field_init）；test-vm 从 295/295 升到 300/300。

- **D-8b-3 Phase 2 / add-default-generic-typeparam**（泛型 type-param `default(T)` 运行时解析）：2026-05-07 落地，由 `docs/spec/archive/2026-05-07-add-default-generic-typeparam/` 实施。
  ...
  实施期阶段 7 发现重大前置缺口 → User 裁决 A：本 spec 同时落地 type_args propagation 基础设施。改造点：① `ObjNewInstr` 增 `TypeArgs` 字段，C# IrGen 在 `EmitBoundNew` 把 `Z42InstantiatedType.TypeArgs` 序列化为 type-tag 字符串列表传给 IR；② zbc/zpkg writer + reader 双端编/解码 type_args；③ Rust `ScriptObject` struct 增 `type_args: Vec<String>` 字段，interp ObjNew handler 在 alloc 后 `borrow_mut` populate；④ JIT 路径 type_args 起初不传（jit_obj_new 签名不变），由后续 spec `expand-jit-type-args`（2026-05-07 落地）补齐：扩 jit_obj_new 末尾两参传 *const String + count，alloc 后 ctor 前 populate `instance.type_args`，JIT 与 interp 行为对齐。zbc 0.8 → 0.9。E0421 gate 改为只对 unknown type 触发；generic-T 走 `DefaultOf` 路径。method-level type-param / free generic / static method on generic class 当前不支持（无 `this` 路径），运行时 graceful 退化为 Null —— 留待后续 spec 拓展 calling convention 携带 type_args。新 3 个 golden（`default_generic_param/`, `default_generic_param_pair/`, `default_generic_param_field_init/`，均带 `interp_only` 标记）+ 2 个 xUnit case 验证。

- **D-8b-1**（stdlib `MulticastException<R>` 泛型类）：2026-05-07 落地，由 `docs/spec/archive/2026-05-07-add-multicast-exception-generic/` 实施。
...
- **D-8b-0**（class arity overloading via shadow registry）：2026-05-07 落地，由 `docs/spec/archive/2026-05-07-add-class-arity-overloading/` 实施。
...
- **D-8b-3 Phase 1**（`default(T)` zero-value 表达式 — fully-resolved T）：2026-05-06 落地，由 `docs/spec/archive/2026-05-06-add-default-expression/` 实施。
...
- **D-8b-2**（catch-by-generic-type 类型过滤）：2026-05-06 落地，由 `docs/spec/archive/2026-05-06-catch-by-generic-type/` 实施。
...
- **fix-cross-zpkg-using-resolution**：2026-05-06 落地，由 `docs/spec/archive/2026-05-06-fix-cross-zpkg-using-resolution/` 实施。

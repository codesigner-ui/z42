...
| `Type.IsEnum` + 接口成员/transitive | `IsClass`/`IsInterface` 已落地（2026-06-16，zbc 1.19 flags bit4 + 接口产最小 TYPE 条目）；`IsEnum` 需 enum 作类型实体设计（独立 design doc）；接口成员枚举 / 接口继承接口 / 数组 IsClass 续作 | [language/reflection.md](design/language/reflection.md#deferred--future-work) |
...
| workload install 后续（B1 命令发现 / B4 平台测试 / B5 mobile publish-run / workload host 校验 / 真机多-slice xcframework）| B2 LOCAL install + B2-4 CI release/manifest 联网装均已落地（2026-06-17）；剩余为后续 change | [toolchain/runtime-workload-distribution.md](design/toolchain/runtime-workload-distribution.md#deferred--待-spec-细化) |

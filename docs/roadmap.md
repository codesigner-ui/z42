...
>
> **z42 编译器源码位置**：`src/libraries/z42.compiler/{core,syntax,project,driver,semantics,ir,pipeline}/` 共 **7 个子包嵌套在 `z42.compiler/` 唯一新增顶级目录下** 1:1 镜像 C# 9 项目结构（z42.Tests / z42.Bench 不需对应子包，按 z42 惯例并入各子包 `tests/` + `bench/`）。zpkg 命名由各子包 manifest 决定（如 `z42.compiler.core.zpkg`），与目录路径无关。driver 是 exe-kind 入口（= 1.0 后的 `z42c.zpkg`），其余 6 个是 lib-kind。0.3.x 实际落地 core/syntax/project/driver 四个；semantics/ir/pipeline 三个空壳占位。workspace `members` glob 加一条 `"z42.compiler/*"` 即可（`Microsoft.Extensions.FileSystemGlobbing` 原生支持多级，零 C# 编译器代码改动）。与 `project_mobile_no_compiler` memory "1.0 后 zpkg 全平台分发" 一致。
>

...
Phase 1（`docs/spec/archive/2026-05-06-add-default-expression/`）落地的 `default(T)` 仅支持 fully-resolved T —— 任何 generic type-parameter（如 `class Foo<R> { R x = default(R); }` 中的 `R`）都被 TypeChecker 用 E0421 InvalidDefaultType 直接拒绝，错误信息明确指向 Phase 2 deferred。
...
- `docs/spec/archive/2026-05-06-add-default-expression/` — Phase 1 设计 / E0421 gate 当前位置

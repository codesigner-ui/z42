...
- [x] 阶段 1: 探索（已完成；前置 = A1/A2/A3/A4 全部落地，invariants 来自 4 个 spec design 段）
...
- [x] P0.1 `gc/region.rs` 加 `#[cfg(debug_assertions)] pub enum Violation`
       (8 variants per design D5)
- [x] P0.2 `Region<T>::validate(&self) -> Result<(), Violation>` 实现
       per design.md "Region::validate sketch"
- [x] P0.3 region_tests.rs 加测试：
...
- [x] P0.4 cargo --lib gc::region GREEN
- [x] P0.5 commit

## P1: ArcMagrGC::debug_validate_invariants + integration + tests (~1 session)

- [x] P1.1 `gc/arc_heap.rs` 加 `debug_validate_invariants` per design
       "ArcMagrGC::debug_validate_invariants" sketch
- [x] P1.2 `collect_cycles` 尾部加 `#[cfg(debug_assertions)] self.debug_validate_invariants()`
- [x] P1.3 `collect_cycles_with_context` 各 mode 分支尾部加同上
- [x] P1.4 单测在 `arc_heap_tests/invariants.rs`:
...
- [x] P2.1 `docs/design/runtime/gc.md` 加新 "Debug invariants" subsection：
       - 列出 8 个 Violation 变种 + 每个的检测时机
       - 何时调用 `debug_validate_invariants`
       - 如何添加新 invariant（开发指南）
       - cfg(debug_assertions) 行为说明
- [x] P2.2 archive 到 `docs/spec/archive/YYYY-MM-DD-add-gc-debug-invariants/`
- [x] P2.3 final `test-all.sh --scope=full` GREEN
- [x] P2.4 commit + push

...
- [x] 1.4 docs/design/stdlib/overview.md：清理 `__sb_*` / `__list_*` / `__dict_*` 等 stale 列表，把 file layout 从 `interp/builtins/` 修正为 `corelib/`，把 intrinsic SoT 委托给审计表
...
- [ ] 4.2 归档到 docs/spec/archive/2026-04-26-extern-audit-wave0/

## 备注

- 实施中扩展了 Scope：发现 `docs/design/stdlib/overview.md` 也含直接受 Wave 0 删除影响的 stale 内容（intrinsic 列表 + file layout + StringBuilder 违规行），按文档同步规则一并修正

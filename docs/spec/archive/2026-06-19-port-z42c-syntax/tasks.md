...
> 状态：🟢 已完成 | 创建：2026-06-08 | 完成：2026-06-19 | 子系统锁：z42c（已释放）
...
- [x] 调研 C# `--dump-tokens`/`--dump-ast` 精确格式 → 设计 z42c dump 格式对齐方案（架构性，先出设计）→ 逐字节对账 CI gate（延后：dump 格式对账低优先；真正的 byte-identical 目标已由 G18 disasm-diff 实现；由后续 port-z42c-self-compile G18 覆盖，2026-06-17）

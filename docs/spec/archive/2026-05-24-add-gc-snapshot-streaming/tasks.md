...
> 状态：🟢 已完成 | 创建：2026-05-24 | 完成：2026-05-24 | 类型：vm
...
- [x] P0.1 MODIFY `src/runtime/src/gc/snapshot.rs`:
       ...
       - Drop `escape_json_str(&str, &mut String)` (no callers after
         refactor)
- [x] P0.2 MODIFY `src/runtime/src/gc/snapshot_tests.rs`:
...
- [x] P0.3 MODIFY `src/runtime/src/corelib/gc.rs`:
       - `builtin_gc_write_heap_snapshot` 改用 `File::create` +
         `BufWriter::new` + `serialize_v8_heapsnapshot_to` + 显式
         `flush()`
- [x] P0.4 `cargo --lib gc::` GREEN（含新 3 unit tests）259 passed
- [x] P0.5 `test-all.sh --scope=full` GREEN（6 stages）
- [x] P0.6 Commit P0
...
- [x] P1.2 Archive 到 `docs/spec/archive/2026-05-24-add-gc-snapshot-streaming/`
- [x] P1.3 Final `test-all.sh --scope=full` GREEN
- [x] P1.4 Commit + push

## 备注

实施期发现：`use std::io::Write as _` 需同时加到 `corelib/gc.rs`（BufWriter
flush 调用）和 `snapshot_tests.rs`（test roundtrip flush 调用）。

...
> **状态**: 📦 已归档快照（2026-05-10）—— 本报告是 2026-05-05 一次性 code & docs review 的存档；其后产生的 spec 已独立追踪在 `docs/spec/archive/2026-05-*/` 各自归档目录。本文件不再作为活规范维护，仅作历史溯源用。
>
> **日期**: 2026-05-05
...
每加一个新表达式/指令要同步改三处。短期接受，但建议在 [docs/design/compiler/compiler-architecture.md](design/compiler/compiler-architecture.md) 记一笔"统一访问者模式或 dispatch 表"作为 L2 后期重构候选——避免被 M7/L3 持续放大。**根本对策见 Part 2 §2.1**。
...
```
docs/spec/changes/add-array-base-class/        # 仅 proposal，无 design/tasks
docs/spec/changes/extend-signature-whitelist/  # design + tasks 齐全，正在实施
```
...
- [generics.md](design/language/generics.md) 1179 行、[project.md](design/compiler/project.md) 964 行、[compiler-architecture.md](design/compiler/compiler-architecture.md) 864 行
...
**建议**: 现在就在 [docs/design/compiler/compiler-architecture.md](design/compiler/compiler-architecture.md) 里画一条线，明确"公共 API surface"是哪几个接口（比如 `ISymbolBinder` / `ITypeInferrer` / `SemanticModel` / `Diagnostic`），新代码逐步迁过去。**不需要现在 public**——只要约定好"以后这些会 public"，避免随手把破坏性改动塞到核心类型里。
...
权衡正确。Roslyn 在大规模代码上验证过该模型。**应在 [compiler-architecture.md](design/compiler/compiler-architecture.md) 显式记录"AST 不可变"作为不变量**，避免后人为了图方便破坏。
...
**建议**: 不急于现在改，但在 [vm-architecture.md](design/runtime/vm-architecture.md) 显式记一笔"未来引入 RID-map 风格的 per-type 懒加载"，避免反射 R-series 设计时假设元数据全量在内存。
...
写进 [vm-architecture.md](design/runtime/vm-architecture.md) 作为不变量，避免后续退化:
...
| **已落地** | `docs/spec/changes/` 两个目录纳入 git（add-array-base-class / extend-signature-whitelist 已归档至 archive/） | (随后续提交) | 防止脱管 | 🟢 2026-05-07 |
...
| **M7 之后** | 在 [compiler-architecture.md](design/compiler/compiler-architecture.md) 记录"AST 不可变 / 三段分相"作为不变量 | docs | Part 3 §3.8、§3.9 防退化 | 📋 |
...
| **M7 之后** | 在 [vm-architecture.md](design/runtime/vm-architecture.md) 记录"VmContext 集中 / Register VM / PinView first-class / static-in-context"作为不变量 | docs | Part 4 §4.正面设计 防退化 | 📋 |
...
13. 在 [compiler-architecture.md](design/compiler/compiler-architecture.md) / [vm-architecture.md](design/runtime/vm-architecture.md) 记录正面设计作为不变量
...
1. **本次提交**: 把 `docs/spec/changes/add-array-base-class/`、`docs/spec/changes/extend-signature-whitelist/` 与本审查报告一并 commit（按 [workflow.md](../.claude/rules/workflow.md) 阶段 9 自动提交规则）

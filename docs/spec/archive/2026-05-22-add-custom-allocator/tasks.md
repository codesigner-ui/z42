...
- [x] 阶段 1: 探索（已完成，map 见 design.md "Architecture"）
...
- [x] P0.1 新建 `gc/region.rs` —— `Region<T>` struct + chunked storage
       (Vec<Box<[MaybeUninit<RegionEntry<T>>; 256]>>)
- [x] P0.2 `RegionEntry<T>` —— value (Mutex), marked (AtomicU8), alive
       (AtomicBool), generation (AtomicU32), finalizer slot
- [x] P0.3 `Region::alloc(value)` —— free list pop 优先，否则 bump pointer
- [x] P0.4 `Region::resolve(handle)` —— `&RegionEntry<T>`，地址稳定
- [x] P0.5 `Region::tombstone(handle)` —— alive=false, generation++,
       push free_list
- [x] P0.6 `Region::iterate_alive(visit)` —— 走 chunks，跳 alive=false
- [x] P0.7 单测 `gc/region_tests.rs`：
...
- [x] P0.8 cargo --lib + region_tests GREEN
- [x] P0.9 commit (Region<T> 独立 + 无 caller，纯 infrastructure)

## P1: GcRef / WeakGcRef rewrite + alloc 路由 (~3 sessions)

- [x] P1.1 `gc/refs.rs` 大改：`GcRef<T>` 从 `Arc<GcAllocation<T>>` 变
       handle (chunk_idx: u16, entry_idx: u16, generation: u32)；
       `Clone` 是 integer copy；`Drop` 是 no-op
- [x] P1.2 `WeakGcRef<T>` 同 handle 形态 + generation check on upgrade
- [x] P1.3 `GcRef::as_ptr` 通过 region 解析到 `RegionEntry.value` 的
       Mutex 地址；保持 pointer stability 契约
- [x] P1.4 `GcRef::borrow` / `borrow_mut` 走 region resolve + Mutex lock
- [x] P1.5 `GcRef::mark` / `is_marked` / `clear_mark` 操作 RegionEntry
       的 `marked` 字段
- [x] P1.6 `ArcMagrGC` 加 `region_object: Region<ScriptObject>` +
       `region_array: Region<Vec<Value>>` 字段
- [x] P1.7 `alloc_object` / `alloc_array` 走 region.alloc
- [x] P1.8 删除 `heap_registry: Vec<WeakRef>` 字段；
       `iterate_live_objects` + `sweep_phase` + `snapshot_*` 改走 region
- [x] P1.9 删除 `GcAllocation<T>` struct（被 RegionEntry 取代）
- [x] P1.10 cargo build green
- [x] P1.11 现有 GC 测试 + interp + JIT + cross_thread 全 GREEN
       （行为应该等价 modulo finalizer timing，那部分留 P2）
- [x] P1.12 commit

## P2: Finalizer 语义迁移 + `Std.GC.Finalize(x)` API (~2 sessions)

- [x] P2.1 删除 `GcAllocation::Drop` 触发 finalizer 路径（GcAllocation
       已被删，但确认 RegionEntry 没继承 Drop-finalizer）
- [x] P2.2 `sweep_phase` 内部 finalize unmarked entries —— 已有从
       mark-sweep spec 落下的代码骨架，确认 region 路径正确触发
- [x] P2.3 新 builtin `builtin_gc_finalize` 在 corelib/gc.rs
- [x] P2.4 注册 `Std.GC.Finalize(x): void` 到 builtin map
- [x] P2.5 `src/libraries/z42.core/src/Std/GC.z42` 加 `Finalize(x)`
       script signature
- [x] P2.6 迁移 `arc_heap_tests/finalization.rs` ~10 个测试：
       - "drop fires finalizer" → "force_collect fires finalizer"
       - 至少 2 个加 `Std.GC.Finalize(x)` 覆盖
- [x] P2.7 审 corelib RAII 模式：
...
- [x] P2.9 cargo + `test-all.sh --scope=full` GREEN
- [x] P2.10 commit

## P3: Bench + docs + archive (~1-2 sessions)

- [x] P3.1 `gc_cycle_bench.rs` 扩 `alloc_throughput_*` workloads：
...
- [x] P3.2 `sweep_overhead_*`：
       - 10k objects survive sweep, measure per-object cost
       - 10k objects swept (with + without finalizer)
- [x] P3.3 跑 baseline (worktree at HEAD~spec-start) vs post-spec
- [x] P3.4 报告入 design.md "Benchmark Results" 段
- [x] P3.5 `docs/design/runtime/vm-architecture.md`：
...
- [x] P3.6 archive 到 `docs/spec/archive/YYYY-MM-DD-add-custom-allocator/`
- [x] P3.7 final `test-all.sh --scope=full` GREEN（双 GcMode）
- [x] P3.8 commit

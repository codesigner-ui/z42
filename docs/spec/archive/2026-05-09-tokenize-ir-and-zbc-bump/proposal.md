...
| 文件 | 类型 | 说明 |
|---|---|---|
| `docs/design/runtime/ir.md` | MODIFY | IR 1.0 字段类型 + Token32 设计 |
| `docs/design/compiler/compilation.md` | MODIFY | zbc 1.0 格式 + IMPT 语义 |
| `docs/design/runtime/vm-architecture.md` | MODIFY | Phase 化路线 → 🟢，Phase 2.D 吸收说明 |
| `docs/roadmap.md` | MODIFY | 进度更新 |
| `.claude/rules/code-organization.md` | MODIFY | 新增 "Reproducible build 规则"段落 |

### 只读引用（理解上下文用，不修改）

- `docs/spec/archive/2026-05-08-introduce-method-token/` — Phase 1 设计参照
- `docs/spec/archive/2026-05-08-formalize-jit-method-token/` — Phase 2 设计参照
- `docs/design/language/language-overview.md` — 类型系统底层

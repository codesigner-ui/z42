...
- [x] 阶段 1: 探索（已完成，map 见 design.md "Architecture" + spec 摸 安全 检查）
- [x] 阶段 2-6: spec 文档（proposal / spec / design / tasks，本文件）
- [x] 阶段 6.5: User 确认
- [x] 阶段 7: 实施 P0-P7
- [x] 阶段 8: 每 phase + 最终 GREEN
- [x] 阶段 9: 归档

## P0: GcMode enum + mode field + dispatch stub (~1 session)

- [x] P0.1 `gc/mod.rs`（或新 `gc/mode.rs`）加 `GcMode` enum
       (`StwMarkSweep` / `ConcurrentMarkSweep`)
- [x] P0.2 `GcMode::from_env()` 解析 `Z42_GC_MODE` env var
- [x] P0.3 `MagrGC` trait 加 `mode() -> GcMode` 默认实现返回 `StwMarkSweep`
       (其它非 Arc heap impls 自动兼容)
- [x] P0.4 `MagrGC` trait 加 `set_mode(GcMode)` 默认实现 panic
       (只 Arc heap 真支持)
- [x] P0.5 `ArcMagrGC` 加 `mode: AtomicU8` 字段；`set_mode` / `mode` 实现
- [x] P0.6 `ArcMagrGC::new()` 从 `GcMode::from_env()` 初始化
- [x] P0.7 `run_cycle_collection` 加 `match self.mode()` dispatch；两个 arm
       都走现有 STW 路径（concurrent arm 暂时也走 STW，证明 dispatch 没
       破坏既有行为）
- [x] P0.8 单测：`mode_default_is_stw_mark_sweep` /
       `set_mode_changes_observable_mode` /
       `env_var_z42_gc_mode_concurrent_selects_concurrent` /
       `env_var_invalid_falls_back_to_stw`
- [x] P0.9 `test-all.sh --scope=full` GREEN (默认 mode 行为零变化)
- [x] P0.10 commit `feat(gc): add-concurrent-gc P0 — GcMode enum + dispatch stub`

## P1: GcPhase enum extension (~0.5 session)

- [x] P1.1 `gc/safepoint.rs` `GcPhase` 加 `ConcurrentMarking` variant
       (在 `Requested` 和 `Marking` 之间)
- [x] P1.2 文档化 phase 状态机：
...
- [x] P1.3 `check_safepoint` 内部 match：`ConcurrentMarking` 时 mutator
       **不** park（这是新行为；其它 phase 都 park 不变）
- [x] P1.4 单测：`gc_phase_concurrent_marking_does_not_park_mutators`
       (使用 mock VmContext 验证 parked_count 不增长)
- [x] P1.5 GREEN + commit

## P2: mark_queue + snapshot helpers (~1 session)

- [x] P2.1 `arc_heap.rs` 加 `mark_queue: Mutex<Vec<Value>>` 字段
- [x] P2.2 helper `mark_if_unmarked(v: &Value) -> bool` (mirror of
       Value variant match — 与 mark_phase 的 BFS 推送共用)
- [x] P2.3 helper `snapshot_roots_into_mark_queue()` — STW 阶段填充 queue
       (调用 pinned_roots + external_root_scanner)
- [x] P2.4 单测：`snapshot_roots_populates_queue_with_marked_roots` 验证
       roots 被 mark + 入 queue
- [x] P2.5 GREEN + commit

## P3: Barrier override branches on mode (~0.5 session)

- [x] P3.1 `arc_heap.rs` `write_barrier_field` / `write_barrier_array_elem`
...
- [x] P3.2 barrier override 加 `debug_assert!(new.is_heap_ref())`
       验证 caller 契约
- [x] P3.3 单测：`barrier_no_op_in_stw_mode` (现有 6 个 write_barriers
       测试在 default mode 下仍 GREEN)
- [x] P3.4 单测：`barrier_shades_new_value_in_concurrent_mode` 验证
       mode==Concurrent 下 mark 命中 + queue push
- [x] P3.5 单测：`barrier_idempotent_on_already_marked_in_concurrent_mode`
       (验证 CAS 失败时不重复 push)
- [x] P3.6 GREEN + commit

## P4: Concurrent mark loop (~2 sessions, 核心 phase)

- [x] P4.1 实现 `run_cycle_collection_concurrent` 函数：
       ...
       - release
- [x] P4.2 `run_cycle_collection` 改为 `match self.mode()` dispatch
       (P0 已 stub，这里填入 concurrent arm 的 真实实现)
- [x] P4.3 单测：`concurrent_collect_preserves_pinned_chain`
       (single-thread sim — collect 当前线程跑，验证 reachable 不被错误清掉)
- [x] P4.4 单测：`concurrent_collect_frees_unreachable_cycle`
       (验证基本 cycle GC 正确性)
- [x] P4.5 单测：`concurrent_collect_with_barrier_marks_new_objects`
       (在 collect 中间 simulated mutator write，验证 barrier shading
       让 new 对象不被错误清掉)
- [x] P4.6 单测：`termination_handshake_drains_final_burst`
       (在 STW handshake 前 simulated burst write，验证 residual 被 drain)
- [x] P4.7 单测：`mode_switch_during_collect_uses_original_mode`
...
- [x] P5.1 现有 `cross_thread_smoke.rs` 跑一遍：`Z42_GC_MODE=concurrent
       cargo test cross_thread`，验证全 GREEN
- [x] P5.2 新加 `arc_heap_tests/concurrent_mark_stress.rs`：
       - 多 mutator 线程持续 alloc + field_set，1 个 collector 周期性
         触发 concurrent collect；跑 N 秒后 assert 无 panic / 无内存爆涨
- [x] P5.3 `test-all.sh --scope=full` 跑两次：
       - default mode → GREEN
...
- [x] P6.1 `benches/gc_cycle_bench.rs` 扩 3 个 concurrent 变种：
...
- [x] P6.2 量化指标：
       ...
       - 与 P4 落地的 STW 总耗时对比
- [x] P6.3 撰写报告入 design.md "Benchmark Results (P6)" 段
- [x] P6.4 commit

## P7: Docs + archive (~0.5 session)

- [x] P7.1 `docs/design/runtime/vm-architecture.md`：
...
- [x] P7.2 archive 到 `docs/spec/archive/YYYY-MM-DD-add-concurrent-gc/`
- [x] P7.3 final `test-all.sh --scope=full` 双 mode GREEN
- [x] P7.4 archive commit + push

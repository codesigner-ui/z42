...
- [x] 阶段 2: SymbolCollector 构造 + TypeChecker 49 处迁移
- [x] 阶段 3: Codegen 14 处 + ImportedSymbolLoader symbol 化
- [x] 阶段 4: BoundCall.Symbol + BoundIndirectCall + 5 visitor 子类
- [x] 阶段 5: TestAttributeValidator + IrGen TestIndex + BoundDumper
- [x] 阶段 6: 文档同步 + 归档
...
- [x] 1.2.6 commit: `feat(compiler): split-symbol-from-type Phase 1 — Symbol interfaces + Z42ClassType cyclic-equality fix`

## 阶段 2: SymbolCollector 构造 IMethodSymbol/IFieldSymbol + TypeChecker 49 处迁移

### 2.1 SymbolCollector 构造 Symbol（携带 Decl）

- [x] 2.1.1 MODIFY [src/compiler/z42.Semantics/TypeCheck/SymbolCollector.Classes.cs](../../../src/compiler/z42.Semantics/TypeCheck/SymbolCollector.Classes.cs):
...
- [x] 2.1.2 MODIFY [src/compiler/z42.Semantics/TypeCheck/SymbolCollector.cs](../../../src/compiler/z42.Semantics/TypeCheck/SymbolCollector.cs):
  - 同上，处理顶层函数 / 接口收集时构造 Symbol
- [x] 2.1.3 MODIFY [src/compiler/z42.Semantics/TypeCheck/SymbolCollector.Impls.cs](../../../src/compiler/z42.Semantics/TypeCheck/SymbolCollector.Impls.cs):
  - impl 块方法构造 `MethodSymbol`

### 2.2 Z42ClassType / Z42InterfaceType 字典值切换

- [x] 2.2.1 MODIFY [src/compiler/z42.Semantics/TypeCheck/Z42Type.cs](../../../src/compiler/z42.Semantics/TypeCheck/Z42Type.cs):
...
- [x] 2.3.1 MODIFY [src/compiler/z42.Semantics/TypeCheck/TypeChecker.Exprs.Members.cs](../../../src/compiler/z42.Semantics/TypeCheck/TypeChecker.Exprs.Members.cs):
  - 24 处 `cls.Methods[name]` / `cls.Fields[name]` 改用 `.Signature` / `.Type` 访问
  - overload resolution 路径检查
- [x] 2.3.2 MODIFY [src/compiler/z42.Semantics/TypeCheck/TypeChecker.Calls.cs](../../../src/compiler/z42.Semantics/TypeCheck/TypeChecker.Calls.cs):
  - 12 处迁移
- [x] 2.3.3 MODIFY [src/compiler/z42.Semantics/TypeCheck/TypeChecker.Exprs.Operators.cs](../../../src/compiler/z42.Semantics/TypeCheck/TypeChecker.Exprs.Operators.cs):
  - 8 处迁移
- [x] 2.3.4 MODIFY [src/compiler/z42.Semantics/TypeCheck/TypeChecker.Generics.cs](../../../src/compiler/z42.Semantics/TypeCheck/TypeChecker.Generics.cs):
  - 3 处迁移
- [x] 2.3.5 MODIFY [src/compiler/z42.Semantics/TypeCheck/TypeChecker.Exprs.cs](../../../src/compiler/z42.Semantics/TypeCheck/TypeChecker.Exprs.cs):
  - 2 处迁移
- [x] 2.3.6 grep 验证：`grep -n "cls\.Methods\[" src/compiler/z42.Semantics/` 检查无遗漏（除 ContainsKey 检查类）
- [x] 2.3.7 `dotnet test` 全绿（含 757 既有 TypeChecker 测试）
- [x] 2.3.8 commit: `refactor(compiler): split-symbol-from-type S2 — SymbolCollector + TypeChecker 49 sites`

## 阶段 3: Codegen 14 处 + ImportedSymbolLoader symbol 化

### 3.1 Codegen 调用点迁移

- [x] 3.1.1 MODIFY [src/compiler/z42.Semantics/Codegen/FunctionEmitterExprs.Members.cs](../../../src/compiler/z42.Semantics/Codegen/FunctionEmitterExprs.Members.cs):
  - 6 处 `cls.Methods.ContainsKey` / `cls.Fields.ContainsKey` 验证 dict 值类型已切换无需更多修改
- [x] 3.1.2 MODIFY [src/compiler/z42.Semantics/Codegen/FunctionEmitterStmts.cs](../../../src/compiler/z42.Semantics/Codegen/FunctionEmitterStmts.cs):
  - 4 处 ContainsKey 检查
- [x] 3.1.3 MODIFY [src/compiler/z42.Semantics/Codegen/FunctionEmitter.cs](../../../src/compiler/z42.Semantics/Codegen/FunctionEmitter.cs):
  - 1 处 `cls.Methods` 访问

### 3.2 ImportedSymbolLoader symbol 构造

- [x] 3.2.1 MODIFY [src/compiler/z42.Semantics/TypeCheck/ImportedSymbolLoader.cs](../../../src/compiler/z42.Semantics/TypeCheck/ImportedSymbolLoader.cs):
  - 三阶段加载内部：构造 `MethodSymbol(decl: null, modifiers: <from TSIG>, span: <from TSIG or Empty>, ...)`
  - 同步 `FieldSymbol(decl: null, ...)` 路径
- [x] 3.2.2 检查 `ExportedClass` / TSIG wire 是否携带 `Modifiers` / `Span` 信息：
  - 如果不携带：先用合理默认（Public + Empty Span），加 TODO 注释；扩 wire format 留独立 follow-up spec
  - 如果携带：直接读
- [x] 3.2.3 MODIFY [src/compiler/z42.Semantics/Synthesis/NativeImportSynthesizer.cs](../../../src/compiler/z42.Semantics/Synthesis/NativeImportSynthesizer.cs):
  - 合成 ClassDecl / FunctionDecl 时同步构造 symbols（类似 ImportedSymbolLoader 但 Decl 非空——因为合成的 AST 节点存在）

### 3.3 验证

- [x] 3.3.1 `dotnet test` 全绿
- [x] 3.3.2 `./scripts/test-vm.sh` 全绿（codegen 行为应零变化）
- [x] 3.3.3 commit: `refactor(compiler): split-symbol-from-type S3 — Codegen + ImportedSymbolLoader symbol-aware`

## 阶段 4: BoundCall.Symbol + BoundIndirectCall + 5 visitor 子类

### 4.1 Bound 节点改造

- [x] 4.1.1 MODIFY [src/compiler/z42.Semantics/Bound/BoundExpr.cs](../../../src/compiler/z42.Semantics/Bound/BoundExpr.cs):
...
- [x] 4.2.1 MODIFY [src/compiler/z42.Semantics/Bound/BoundExprVisitor.cs](../../../src/compiler/z42.Semantics/Bound/BoundExprVisitor.cs):
...
- [x] 4.2.2 MODIFY [src/compiler/z42.Semantics/Codegen/FunctionEmitterExprs.cs](../../../src/compiler/z42.Semantics/Codegen/FunctionEmitterExprs.cs):
  - `IrEmitExprVisitor.VisitIndirectCall`：发射 indirect call IR（复用 `EmitBoundCall.Indirect` 路径或拆出新 helper）
- [x] 4.2.3 MODIFY [src/compiler/z42.Semantics/TypeCheck/FlowAnalyzer.cs](../../../src/compiler/z42.Semantics/TypeCheck/FlowAnalyzer.cs):
  - `ReadsVisitor.VisitIndirectCall`：递归 Callee + Args（每个都 `Visit()`）
- [x] 4.2.4 MODIFY [src/compiler/z42.Semantics/TypeCheck/ClosureEscapeAnalyzer.cs](../../../src/compiler/z42.Semantics/TypeCheck/ClosureEscapeAnalyzer.cs):
  - `EscapeExprScanner.VisitIndirectCall`：Callee 走 **非 callee 位置**（`calleePosition: false`）—— 间接调用的 callee 是个值，不是方法，应该 escape；Args 同样
- [x] 4.2.5 MODIFY [src/compiler/z42.Semantics/Codegen/FunctionEmitter.cs](../../../src/compiler/z42.Semantics/Codegen/FunctionEmitter.cs):
  - `ClassRefScanner.VisitIndirectCall`：override 为 no-op（preserve 旧 BoundCall 间接路径不递归的语义）
- [x] 4.2.6 MODIFY [src/compiler/z42.Pipeline/BoundDumper.cs](../../../src/compiler/z42.Pipeline/BoundDumper.cs):
...
- [x] 4.3.1 MODIFY [src/compiler/z42.Semantics/TypeCheck/TypeChecker.Calls.cs](../../../src/compiler/z42.Semantics/TypeCheck/TypeChecker.Calls.cs):
...
- [x] 4.4.1 `dotnet test` 全绿（含 closures / lambda 测试）
- [x] 4.4.2 `./scripts/test-vm.sh` 全绿（IR 输出应等价 —— 间接调用走 CallIndirect，直接调用走 Call/VCall）
- [x] 4.4.3 commit: `refactor(compiler): split-symbol-from-type S4 — BoundCall.Symbol + BoundIndirectCall + visitor`

## 阶段 5: TestAttributeValidator + IrGen TestIndex + BoundDumper

### 5.1 TestAttributeValidator 通过 Symbol 访问

- [x] 5.1.1 MODIFY [src/compiler/z42.Semantics/TestAttributeValidator.cs](../../../src/compiler/z42.Semantics/TestAttributeValidator.cs):
...
- [x] 5.2.1 MODIFY [src/compiler/z42.Semantics/Codegen/IrGen.Generate.cs](../../../src/compiler/z42.Semantics/Codegen/IrGen.Generate.cs):
...
- [x] 5.3.1 NEW [src/compiler/z42.Tests/SymbolLayerTests.cs](../../../src/compiler/z42.Tests/SymbolLayerTests.cs):
...
- [x] 5.3.2 NEW [src/compiler/z42.Tests/SymbolDeclSpanTests.cs](../../../src/compiler/z42.Tests/SymbolDeclSpanTests.cs):
...
- [x] 5.4.1 BoundDumperTests.cs 扩展用例：`BoundCall_Dump_ShowsSymbolDeclLocation`
- [x] 5.4.2 既有 BoundDumperTests 5 用例可能因为新增 `decl=...` 标记导致字符串断言失败 → 更新断言

### 5.5 验证

- [x] 5.5.1 `dotnet test` 全绿（含新增 ~12 测试）
- [x] 5.5.2 `./scripts/test-vm.sh` 全绿
- [x] 5.5.3 commit: `refactor(compiler): split-symbol-from-type S5 — TestAttributeValidator + IrGen + dumper symbol-aware`
...
- [x] 6.2 MODIFY [src/compiler/z42.Semantics/README.md](../../../src/compiler/z42.Semantics/README.md):
  - 核心文件表加 `Symbols/IMemberSymbol.cs` / `IMethodSymbol.cs` / `IFieldSymbol.cs`
- [x] 6.3 MODIFY [docs/review.md](../../../docs/review.md):
  - Part 2 §2.3 + Part 3 §3.1 状态 📋 → 🟢 2026-05-10
  - 优先级清单清空（split-symbol-from-type 是最后一项）
  - 修订记录追加
- [x] 6.4 tasks.md 状态改 🟢 已完成
- [x] 6.5 移动 `docs/spec/changes/split-symbol-from-type/` → `docs/spec/archive/2026-05-10-split-symbol-from-type/`
- [x] 6.6 commit: `docs+spec(compiler): split-symbol-from-type — archive + architecture doc sync`
- [x] 6.7 push origin main

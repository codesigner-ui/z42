...
D-8b-3 Phase 2（`docs/spec/archive/2026-05-07-add-default-generic-typeparam/`）让泛型 type-param `default(T)` 通过运行时 `instance.type_args` 解析到具体 zero-value。落地时 JIT 路径上的 `jit_obj_new` 没扩展签名，JIT-allocated 实例 `type_args` 字段始终为空 → `default(T)` 在 JIT 模式下退化为 `Value::Null`，与 interp 模式不一致。

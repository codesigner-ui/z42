...
- [x] P0.1 `RegionEntry<T>` 加 `gen_age: AtomicU8` 字段（默认 0）
- [x] P0.2 `Region<T>` 加 `young_list: Vec<(u16, u16)>` 字段
- [x] P0.3 `Region<T>` 加 `card_dirty: Vec<u32>` 字段（per-chunk bit）
- [x] P0.4 `Region::alloc` 修改：push 到 `young_list`；free-list pop 路径
       也 reset gen_age 到 0 + push young_list（重用槽即新对象）
- [x] P0.5 `Region::promote(handle)` 实现：gen_age++，到达阈值 (2)
       后从 young_list 移除
- [x] P0.6 `Region::iterate_young(visit)` —— 走 young_list 直接 index
       到 chunks（O(young) 不是 O(total)）
- [x] P0.7 `Region::mark_card_dirty(chunk_idx)` —— bit set in `card_dirty`
- [x] P0.8 `Region::is_card_dirty(chunk_idx)` + `Region::clear_card_dirty()`
- [x] P0.9 `Region::iterate_dirty_cards(visit)` —— yields all entries
       in dirty chunks (不 filter gen_age，让 minor GC 自己判断)
- [x] P0.10 `Region::tombstone_via_entry` 同步移除 entry 在 young_list
       中的位置（若是 young）
- [x] P0.11 region_tests.rs 加测试：
...
- [x] P0.12 `GcRef::gen_age()` accessor 在 refs.rs（atomic load）
- [x] P0.13 cargo --lib + region_tests + refs GREEN
- [x] P0.14 commit

## P1: GcMode::GenerationalMarkSweep + barrier override (~2 sessions)

- [x] P1.1 `gc/mode.rs` 加 `GcMode::GenerationalMarkSweep` variant
- [x] P1.2 `GcMode::from_env()` 解析 `Z42_GC_MODE=generational`
- [x] P1.3 `GcMode::from_u8` 加新 discriminant 映射
- [x] P1.4 ArcMagrGC write_barrier_field override 加 GenerationalMarkSweep
       分支：检查 `owner.gen_age >= 1 && new.gen_age == 0` → mark card
- [x] P1.5 同上 write_barrier_array_elem
- [x] P1.6 单测：
...
- [x] P1.7 行为 parity 验证：StwMarkSweep 模式下所有现有 174 GC 测试
       通过（young_list bookkeeping inert 不影响）
- [x] P1.8 commit

## P2: Minor GC 分发 + 老→新 root 扫描 + promotion (~3 sessions)

- [x] P2.1 `ArcMagrGC` 实现 `collect_minor()`：
       ...
       - clear_card_dirty
- [x] P2.2 `collect_cycles_with_context` 加 `GenerationalMarkSweep` 分支：
       默认走 collect_minor；触发条件命中 major 走 collect_major
- [x] P2.3 `sweep_phase_young_only` 实现：iterate young_list，
       tombstone unmarked + fire finalizer + remove from young_list
- [x] P2.4 promote_survivors 实现：iterate young_list，对每个 alive +
...
- [x] P3.1 `collect_major()` 实现：basically existing
       `run_cycle_collection_stw` rebadged + 显式 clear_card_dirty 在结尾
- [x] P3.2 `should_escalate_to_major(minor_survival_rate: f32) -> bool`
       heuristic：>= 0.75 触发 major
- [x] P3.3 `collect_cycles_with_context` 在 minor 完成后检查 escalation
- [x] P3.4 `maybe_auto_collect`/`needs_auto_collect` 加 young pressure
       判断：young_list capacity 满足条件触发 minor；否则按既有 old
       pressure 触发 major
- [x] P3.5 env var `Z42_GC_TENURE` (promotion threshold, default 2)
...
- [x] P3.7 集成测试：`Z42_GC_MODE=generational test-all.sh --scope=full`
       6 stages GREEN
- [x] P3.8 commit

## P4: Bench + docs + archive (~1-2 sessions)

- [x] P4.1 `gc_cycle_bench.rs` 扩 generational workloads：
...
- [x] P4.2 报告入 design.md "Benchmark Results" 段
- [x] P4.3 `docs/design/runtime/vm-architecture.md`：
...
- [x] P4.4 archive 到 `docs/spec/archive/YYYY-MM-DD-add-generational-gc/`
- [x] P4.5 final `test-all.sh --scope=full` 三 mode GREEN (stw, concurrent, generational)
- [x] P4.6 commit

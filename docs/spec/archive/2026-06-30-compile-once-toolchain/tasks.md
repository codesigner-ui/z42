...
- [→P2] 1.4 **条件不动点 helper**（gen1 vs gen2 对，等跳 gen3／不等验 {gen2}=={gen3}）——**移到 P2**：现有 `_testSelfHostByteIdentical` 已覆盖稳态 gen1==gen2；条件 gen3 的失败路径只在「改 codegen」时触发，本地无 codegen-diff 难验，落在 P2 compile job（真有 SDK→gen1→gen2 序列）里实现+验证更自然。

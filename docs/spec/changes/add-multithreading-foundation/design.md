...
### Decision 8: GC scanner cycle 避免 + 多线程 root scanning 推迟 (Phase 2.2 implementation discovery, 2026-05-19)

**问题**：原 design 假设 heap 进 VmCore 后 scanner closure 仍捕获 `Arc<VmCore>`。但 `VmCore.heap` 包含 scanner，scanner 强引 `VmCore` → 形成循环 → VmCore 永不 drop。

**选项**：

- **A** Scanner 改捕获 `Weak<VmCore>`：每次扫之前 upgrade。Per-thread roots 仍走 `Rc<RefCell>` clone（current pattern）。**单 VmContext per VmCore 不变量**确保 per-thread state 可达
- **B** VmCore 加 VmContext 注册表（`Mutex<Vec<Weak<VmContext>>>` 或 raw ptr + RAII drop）：scanner walk 注册表扫所有线程 frames。终态多线程必需

**决定**：**A**（Phase 2.2 范围内）+ **B 推迟到独立 spec**。理由：

1. 当前 spec 范围是 single-threaded foundation；scanner 只需看到一份 per-thread state，Rc clone 足够
2. B 的实现复杂度（VmContext 地址稳定性 / 注册时机 / Drop safety）远超 Phase 2.2 估算
3. B 正确实现需先决定 VmContext 是否走 Arc 内嵌（newtype `VmContext(Arc<VmContextInner>)`）—— 大 API 改动，独立 spec
4. `MagrGC` trait 加 `set_external_root_scanner` 默认 no-op —— 让 `Box<dyn MagrGC>` 也能 install scanner（核心 trick：先 `Arc::new(VmCore { heap: ... })` 再 `Arc::downgrade` 喂给 scanner closure）

**后续 spec**：`add-vmcontext-registry` —— 必须先于 `add-threading-stdlib` 落地。Scope 包含：VmContext 地址稳定方案（Pin / Arc 内嵌 / 注册 ptr 三选一）、registry Drop safety、scanner 改 walk 注册表。

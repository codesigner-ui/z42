...
| `toolchain` | **impl-workload-install（B2-1 进行中）** | 2026-06-17 | workload 包格式 + xtask 产包；B2-2 本地 install --from + 按需拉 runtime；B2-3 CI。（前序 **~~rework-desktop-publish-run~~** ✅ 已归档 2026-06-17（定稿命令模型：字节码 build / run 双形态 / publish / export(仅ios·android) / test；`export desktop`→`publish desktop` + 新增 `run desktop`；GREEN 含两命令运行级实跑）。前序 **~~remove-host-toplevel S5~~** ✅ / **~~add-desktop-export~~** ✅ / **~~migrate-facades-to-workload S3'~~** ✅ / **~~migrate-tier2-to-workload S1~~** ✅ / **consolidate-platform-into-workload S0** ✅。**B 子系统余下：B1 命令发现 / B2 打包+auto-runtime / B4 测试改 workload / B5 mobile publish/run 生命周期** |

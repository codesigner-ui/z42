...
| `toolchain` | —（空闲）| — | **impl-workload-install B2-1 iOS + B2-3 wasm** ✅（runtime/workload 分包 + `z42 workload install/list/remove`；iOS 端到端 `swift build` ✓ / wasm 端到端 `node import` ✓；install 平台分支：ios 改写 Package.swift / wasm symlink pkg-*）。前序 **~~rework-desktop-publish-run~~** ✅（命令模型定稿）/ **~~remove-host-toplevel S5~~** ✅ / **~~add-desktop-export~~** ✅ / **~~migrate-facades-to-workload S3'~~** ✅ / **~~migrate-tier2-to-workload S1~~** ✅ / **consolidate S0** ✅。**B 余下：B2-3 android/wasm 拆 + B2-4 CI manifest / B1 命令发现 / B4 测试改 workload / B5 mobile 生命周期** |

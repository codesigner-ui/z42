...
| `toolchain` | **remove-examples-from-packaging** | 2026-06-20 | 各发行包不再含 examples/（examples/ 源目录保留为测试夹具）。队列下一个：🟡 **fix-desktop-runtime-embed-libs** SPEC READY（桌面 runtime 包补 native/ 嵌入件 + headers）。前序：**~~clarify-launcher-command-face~~ ✅ 已归档 2026-06-17**（实装 `z42 list --workloads`；命令面 `update`/`use`/`workload update` 标 planned——真特性需 latest/pin resolution 未建）。**~~dry-ci-release-index~~ ✅ 已归档 2026-06-17** |

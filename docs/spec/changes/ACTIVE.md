...
| `toolchain` | —（空闲）| — | **~~dry-ci-release-index~~ ✅ 已归档 2026-06-17**（抽 release.yml ↔ ci.yml(publish-nightly) 重复的 assemble-desktop-workload + release-index 生成到 `scripts/release/{assemble-desktop-workload,gen-release-index}.sh`（label 参数化）；两 workflow 改调脚本。两路输出**逐字节等价**重构前 inline jq（diff 验）；去 hand-sync 债 + 根除 schema 漂移源）。**~~unify-platform-deploy-rid~~ ✅ 已归档 2026-06-17**（① publish/export/run 部署形态 → `<toml> --rid <rid>` 去平台子命令；② workload remove→uninstall；③ desktop workload 合一 RID-agnostic 携全 RID apphost-<rid> → publish --rid 跨平台输出。macos-arm64 本地全验：publish/run --rid → hello,world / 门控 / 跨产 stub 选择 / uninstall；CI-sim 4 片合并单 archive + gate exit0。macos 目标需 macos codesign 入 Deferred）。**~~add-desktop-workload~~ ✅ 已归档 2026-06-17**（Decision 9 完全门控：apphost stub 从 SDK/launcher baked 移入 desktop workload，per-host-RID `z42-workload-<v>-desktop-<rid>`；`publish desktop` 门控于 `workload install desktop`；manifest `workloads.desktop.hosts.<rid>` + runtimes:[]。macos-arm64 本地全程验：SDK 无 apphost / 门控报错 / 装后 publish 产 apphost exe **跑出 hello,world** / 联网 per-host install + wrong-host neg / CI-sim 4 归档+gate exit0）。**~~rename-sdk-workload-archives~~ ✅ 已归档 2026-06-17**（发布归档加 kind 前缀消歧：SDK→`z42-sdk-<v>-<rid>`、workload→`z42-workload-<v>-<wl>`；runtime/launcher 不动。release.yml+ci.yml archive/index + launcher 2 + install-z42.{sh,bat} 2 处 manifest-absent fallback + docs schema/launcher.md。消费端从 manifest 读名→自动跟随。CI-sim 16 归档/gate exit0 ✓）。**~~fix-ci-package-split-layout~~ ✅ 已归档 2026-06-17**（修 ci.yml：Verify iOS/Android/wasm 步从旧扁平布局改对分包——tooling 验 gradle/SwiftPM/npm 工程 + runtime pack 验 native libs/headers/libs；publish-nightly archive 平台分支产 runtime pack + workload tooling 双归档 + index 加 workloads/host 段。Android Verify 对在手真包验通过；nightly archive+index sim 16 归档/dedup/exit0）。**~~add-workload-host-gate~~ ✅ 已归档 2026-06-17**（`workloads.<wl>.host` 落地：CI emit host RID 列表 ios=[macos-arm64]/android=4桌面/wasm=["*"]；launcher 联网装前 `_hostAllowed` gate，不通过即拒不下载；缺失=不限制向后兼容；本地 `--from` 不 gate。本地 mock e2e 四态全过）。**~~add-workload-manifest-install B2-4~~ ✅ 已归档 2026-06-17**（CI release.yml 平台双归档 tooling+runtime pack + `release-index.json` `workloads` 段；launcher `workload install <wl>`（无 `--from`，可 `--base-url`）联网装：`_fetchWorkloadEntry` + `_downloadVerified` + 遍历 runtimes 拉 pack + `_bedRuntimeIntoWorkload`。**本地 mock e2e（android 多-RID）权威验证**：tooling+2 pack 下载+sha+解压+铺设 → gradlew AAR ✓；缺 workload/篡改 sha 两反例 ✓；CI correct-by-construction（下次 tag 验）。dotnet 1568/0）。**~~impl-workload-install B2~~ ✅ 已归档 2026-06-17**（三平台 LOCAL produce + install：runtime/workload 分包 + `z42 workload install/list/remove`；iOS `swift build` ✓ / wasm `node import` ✓ / android gradlew AAR ✓；`--from` 可选 → 一 workload 多 RID 增量装）。前序 **~~rework-desktop-publish-run~~** ✅（命令模型定稿）/ **~~remove-host-toplevel S5~~** ✅ / **~~add-desktop-export~~** ✅ / **~~migrate-facades-to-workload S3'~~** ✅ / **~~migrate-tier2-to-workload S1~~** ✅ / **consolidate S0** ✅。**B 余下（后续 change，入 roadmap Deferred）：B1 命令发现 / B4 测试改 workload / B5 mobile 生命周期 / workload host 校验 / 真机多-slice xcframework** |

...
| `stdlib` | add-reflection-mvp ‖ add-toml-key-order | 2026-06-08 | 反射 MVP：z42.core 扩展 Type + Std.Reflection 类（feat 30776fae 已提交，待归档释放）。**例外共存**：add-toml-key-order 仅动 `z42.toml`（TomlWriter/TomlValue + test），与反射 `z42.core` 零重叠。（add-uri-ipv6-host / add-binary-float / compression-decoder-pull-mode / add-file-last-write-time / add-directory-copy 均曾共存，已归档释放）|
...
| add-toml-key-order | stdlib（仅 z42.toml；纯 z42，与 reflection 例外共存）|

...
| `runtime` | add-reflection-mvp ‖ add-binary-float | 2026-06-08 | 反射 MVP：corelib reflection builtins + TypeDesc 句柄化。**例外共存**：add-binary-float 加 `corelib/convert.rs` 4 个 bit-cast builtin + `mod.rs` convert 区 4 行（line~137，与反射的 line 34+205 区段分离，`git apply --cached` 隔离）。（add-file-last-write-time 曾共存，已归档释放）|
| `stdlib` | add-reflection-mvp ‖ add-binary-float | 2026-06-08 | 反射 MVP：z42.core 扩展 Type + Std.Reflection 类。**例外共存**：add-binary-float 仅动 `z42.io.binary`（BinaryReader/Writer + 1 test），与反射 `z42.core` 零重叠。（compression-decoder-pull-mode / add-file-last-write-time / add-directory-copy 均曾共存，已归档释放）|
...
| add-binary-float | runtime + stdlib（convert.rs 4 builtin + z42.io.binary；与 reflection 例外共存，文件/区段分离）|

...
| `toolchain` | port-z42c-core **(+ add-apphost 并行)** | 2026-06-07 | xtask test compiler-z42 接入 z42-test-runner 跑 z42c [Test]。<br>⚠️ **并行例外（2026-06-09，User 授权）**：`add-apphost` 与本 change 并行占 `toolchain`——port-z42c-core 已无代码改动（仅剩 verify+archive，14/17），且二者文件不重叠（port=xtask test-runner，apphost=launcher crate `src/`+`core/`）。风险经评估近零；各自归档时释放。 |
...
| add-apphost | toolchain（**实施中**，2026-06-09：User 授权与 port-z42c-core 并行占 toolchain，见持有表并行例外说明）|

...
> 状态：🟡 进行中 | 创建：2026-05-30 | 类型：lang + ir + stdlib (full flow)

## 进度概览

- [ ] 阶段 1: AttributeArg discriminator + parser int-literal 支持
- [ ] 阶段 2: 现有 [Skip] / [Native] 消费侧切到 RequireStringArg
- [ ] 阶段 3: [Timeout] AttributeBinder + TestAttributeValidator (E0916)
- [ ] 阶段 4: TestEntry + ZbcWriter minor bump + ZpkgWriter 联动
- [ ] 阶段 5: Rust zbc_reader 同步 minor + 读 timeout_ms
- [ ] 阶段 6: test-runner exec.rs per-test budget + 2× ceiling clamp
- [ ] 阶段 7: 测试（C# parser/binder/validator + Rust integration + stdlib 示范）
- [ ] 阶段 8: 文档同步（zbc.md / zpkg.md changelog / testing.md / z42.test README）
- [ ] 阶段 9: Fixture regen + GREEN + commit + archive

## 阶段 1: AttributeArg discriminator + parser int-literal

- [ ] 1.1 `src/compiler/z42.Syntax/Parser/Ast.cs` —
...
- [ ] 1.2 `src/compiler/z42.Syntax/Parser/TopLevelParser.Helpers.cs` —
...
- [ ] 1.3 同上：把 `"Timeout"` 加入 `TestAttributeNames` 白名单
- [ ] 1.4 任何现有 unit test 直接构造 `TestAttribute(...)` 字面量传 dict
  的，更新到新 AttributeArg 形态（多半在 z42.Tests 一两处）

## 阶段 2: 现有消费侧迁移

- [ ] 2.1 `src/compiler/z42.Semantics/TestAttributeValidator.cs` 加
...
- [ ] 2.2 同上：`hasSkip && skipAttr` 分支的 reason 读取改用 helper
- [ ] 2.3 `src/compiler/z42.Semantics/Codegen/IrGen.Tests.cs`
...
- [ ] 2.4 任何使用 `[Native(lib: "...", entry: "...")]` 的解析处也走
  同样 string-arg helper（位置：`ParseNativeAttributeBody` 上游消费侧）
- [ ] 2.5 `dotnet build src/compiler/z42.slnx` 通过；`dotnet test`
  `--filter "FullyQualifiedName~TestAttribute"` 全绿（旧测试无回归）

## 阶段 3: [Timeout] AttributeBinder + Validator

- [ ] 3.1 `src/compiler/z42.Core/Diagnostics/Diagnostic.cs` —
  加 `public const string TimeoutValueInvalid = "E0916";`（紧跟 E0915
  `SetupTeardownSignatureInvalid` 之后）
- [ ] 3.2 `src/compiler/z42.Core/Diagnostics/DiagnosticCatalog.cs` —
  E0916 entry：name "TimeoutValueInvalid"，category Test，description
  覆盖六种 trigger
- [ ] 3.3 `src/compiler/z42.Semantics/TestAttributeValidator.cs` —
  - `ValidateFunction` 局部加 `bool hasTimeout = false; TestAttribute?
    timeoutAttr = null;` 累积
  - switch case 加 `"Timeout"` → 累积；若已有 → 立即 E0916 "applied
    more than once"
  - 校验链末尾加 `if (hasTimeout && timeoutAttr is not null)`
    分支调 `ValidateTimeoutAttribute`：
    - 无 `[Test]` 也无 `[Benchmark]` → E0916 "requires [Test] or
      [Benchmark]"
    - `NamedArgs` 缺 `milliseconds` key → E0916 "requires a single
      named arg \"milliseconds: <int>\""
    - value 不是 `AttributeArgInt` → E0916 "must be an integer literal
      (got string)"
    - `<= 0` → E0916 "must be > 0 (got <v>)"
    - `> int.MaxValue` → E0916 "must fit in i32 (got <v>)"

## 阶段 4: TestEntry + ZbcWriter

- [ ] 4.1 `src/compiler/z42.IR/TestEntry.cs` — `TestEntry` record
  追加 `int TimeoutMs` 字段（默认 0），放在 `TestCases` 之后；XML
  doc 注明 "0 = no override, runner uses default"
- [ ] 4.2 `src/compiler/z42.Semantics/Codegen/IrGen.Tests.cs` —
  `BuildTestEntry`：
  - 加 `int timeoutMs = 0;`
...
- [ ] 6.3 引用 `TEST_TIMEOUT_SECS` 的其他地方（如 doc comment）一并
  改名 / 删除

## 阶段 7: 测试

- [ ] 7.1 `src/compiler/z42.Tests/AttributeArgIntParseTests.cs` — NEW
...
- [ ] 7.2 `src/compiler/z42.Tests/TimeoutAttributeBinderTests.cs` — NEW
  - `[Test][Timeout(milliseconds: 5000)]` → TestEntry.TimeoutMs == 5000
  - `[Test][Timeout]` (无参) → E0916 "requires \"milliseconds: <int>\""
  - `[Test][Timeout(milliseconds: 0)]` → E0916 "must be > 0"
  - `[Test][Timeout(milliseconds: "5000")]` → E0916 "must be integer"
  - `[Timeout(milliseconds: 1000)]` 无 `[Test]` → E0916 "requires [Test]"
  - `[Test][Timeout(milliseconds: 1000)][Timeout(milliseconds: 2000)]`
    → E0916 "applied more than once"
- [ ] 7.3 `src/toolchain/test-runner/tests/timeout_per_test_test.rs` — NEW
...
- [ ] 7.4 `src/libraries/z42.crypto/tests/ecdsa_secp256k1_vectors.z42`
  — MODIFY，`test_secp256k1_sign_verify_round_trip` 之上加
  `[Timeout(milliseconds: 60000)]`，作 in-tree 示范

## 阶段 8: 文档同步

- [ ] 8.1 `docs/design/runtime/zbc.md` — Minor changelog 加一行
  （minor 新值 / 日期 / 触发 spec / 引入字段 `TestEntry.timeout_ms i32`）
- [ ] 8.2 `docs/design/runtime/zpkg.md` — 同上（联动 zbc minor 的注解）
- [ ] 8.3 `docs/design/testing/testing.md` —
  - "R-series 待落地" 表去掉 `[Timeout]` 行
  - 新增 §"Per-test timeout"（精简版 spec.md 内容）
  - 提一行 "attribute named-arg values support integer literals as
    of this spec"
- [ ] 8.4 `src/libraries/z42.test/README.md` — 能力表加
  `[Timeout(milliseconds: N)]` 行

## 阶段 9: Fixture regen + GREEN + commit + archive

- [ ] 9.1 `./src/tests/zbc-format/generate-fixtures.sh` 跑一遍；git
  diff 应只含 minor 字节 + 新 timeout_ms 字段（默认 0）
- [ ] 9.2 `./src/tests/zpkg-format/generate-fixtures.sh` 跑一遍同上
- [ ] 9.3 `./scripts/regen-golden-tests.sh --release` 把 stdlib + 测试
  zbc 一并 regen
- [ ] 9.4 `dotnet test --filter "FullyQualifiedName~Z42.Tests.Zbc|FullyQualifiedName~Z42.Tests.Zpkg"`
  全绿（format invariant + golden）
- [ ] 9.5 `./scripts/test-all.sh --parallel --jobs=4` 全绿
- [ ] 9.6 commit + push（spec 文件 + 实现 + 文档 + fixture diff
  一并；单 commit）
- [ ] 9.7 归档：`docs/spec/changes/add-test-timeout-attribute/` →
...
- C# parser 数字字面量 i32 vs i64 表示：`AttributeArgInt.Value` 用
  `long` 容纳 lexer 产出，在 [Timeout] 消费侧再 narrow + E0916
  报溢出

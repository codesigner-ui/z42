# Spec: wasm SDK package（wasm32，含 staticlib + cdylib）

> 落地 1 个 wasm package：`z42-<v>-wasm32-<config>`。同时含 staticlib（libz42.a wasm32 object archive）+ cdylib（z42_wasm_bg.wasm + JS glue）。

## ADDED Requirements

### Requirement 1: wasm32 package 产物

#### Scenario: --rid wasm32 产包
- **WHEN** `./scripts/package.sh release --rid wasm32`
- **THEN** `artifacts/packages/z42-<v>-wasm32-release/` 存在，含 bin/libs/native/examples/manifest.toml 5 项 + 平台原生入口
...
- **WHEN** 读 `z42-<v>-wasm32-release/examples/hello_c/README.md`
...
- **WHEN** 读 `z42-<v>-wasm32-release/manifest.toml`
- **THEN** `[package].rid = "wasm32"` + `[contents.platform]` 含 `npm-manifest = "package.json"` + `wasm-bindgen = ["pkg-web", "pkg-nodejs"]` + `[compat].wasm-bindgen-version = "0.2"`
...
**After:** 保留 in-repo；附加导出 wasm32 package 到 `artifacts/packages/`。staticlib 是新增产物（之前未产）。

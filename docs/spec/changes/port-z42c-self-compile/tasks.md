...
- [ ] G17 z42c.ir 逐包对账（进行中，复用 G10-G16 经验；dual-build@0.20 一致工具链下首测）：
  ...
  - [ ] **G17b FQ 命名空间解析根因**（STRS 主体 +2430 字节，~64 串，未修）：`is`/`as`/static-get 引用**同包但声明在兄弟 ns 的本地类**（如 `ConstI32Instr`/`IrType` 声明于 `Z42.IR`，从 `Z42.IR.BinaryFormat` 的 ZbcInstr.z42 经 `using Z42.IR;` 引用）时，z42c `QualifyClass`/`Qualify` 误用**引用方 ns**（`Z42.IR.BinaryFormat.ConstI32Instr`）；C# 经 `ImportedClassNamespaces`（含 `using` 引入的跨 ns 同包类）解析到**声明 ns**（`Z42.IR.ConstI32Instr`）。**根因**：z42c 的 `IrGen.ImportedClassNs` 仅含跨-zpkg 导入，**漏同包 sibling-ns 类**（C# 的 `ImportedClassNamespaces` 含之）。修：semantics 在按 CU `using` 解析时把同包其它 ns 的类登记进 ImportedClassNs（短名→声明 ns）。z42c.core 未触发因其引用与声明同 ns（巧合）。

...
- [ ] G17 z42c.ir 逐包对账（进行中，复用 G10-G16 经验；dual-build@0.20 一致工具链下首测）：
  ...
  - [ ] **G17c z42c.ir 残差**（剩 17B，独立小 bug，follow-up）：① STRS `byte` vs `u8[]`（z42c 某处发 `byte`，C# 发 `u8[]`——类型名 canon/数组元素名）② DEPS 串 `z42.core.zpkg`(z42c) vs `z42.cli.zpkg`(C#)（dep 文件名分歧，待查）③ `__double_to_bits`（C# 发此 builtin，z42c 漏/异）④ EXPT +5 / SIGS +22 / MODS −33（结构小差，逐段 cmp 定位）。

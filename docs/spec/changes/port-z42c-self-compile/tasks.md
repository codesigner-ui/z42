...
- [ ] G17 z42c.ir 逐包对账（进行中，复用 G10-G16 经验；dual-build@0.20 一致工具链下首测）：
  ...
  - [ ] **G17c-2 z42c.ir prim 名 canon（需 IR/TSIG 双路协调，未落地）**：根因已定位——C# **两套类型名**：① **编译 IR**（SIGS/TYPE/FUNC ArrayNew）用 `Z42Type.ToString()`=**TypeRegistry 规范名**（`byte→u8`/`sbyte→i8`/`short→i16`/`ushort→u16`/`uint→u32`/`ulong→u64`；`int`/`long`/`float`/`double`/`bool`/`char`/`string` **保留源名**——非全 canon！见 `TypeRegistry.cs` 各 TypeEntry 首字段）；② **TSIG**（导出签名）用 `TypeExprToString(AST)`=**原始源拼写**（`byte[]`）。**同一 ToBytes 返回 byte[] → C# SIGS=`u8[]`(idx249) + TSIG=`byte[]`(idx692) 两份**。z42c `ResolveType` 产**裸源名**（`byte`）→ 两路都裸 → SIGS 错(`byte[]`应`u8[]`)、TSIG 对(`byte[]`巧合)。**试 canon ResolveType（byte→u8）→ SIGS 对但 TSIG 破**（z42c TSIG 也走 `.Name()`→变 `u8[]`，应留 `byte[]`）→ 已回退（净零，只挪错）。**正解需双路协调**：IR 侧 canon（`.Name()` 规范名）+ TSIG 侧改用**原始 AST 拼写**（z42c ExportedTypeExtractor 的 `sig.Ret/ParamTypes/FieldType.Name()` → `md.RetType/Params[i].Type/fd.Type.Dump()`）。⚠️ 风险：`.Dump()` vs `.Name()` 对非-byte（泛型/nullable/限定名）须逐一验 byte-identical（现 TSIG 用 `.Name()` 裸名恰好对齐 C# TypeExprToString，切 `.Dump()` 须确认等价）→ 独立聚焦迭代。
  - [ ] **G17c-3 z42c.ir 末尾残差**：① DEPS 串 `z42.core.zpkg`(z42c) vs `z42.cli.zpkg`(C#)（dep provider 分歧，疑 harness 两侧 libs 目录差或 DepIndex first-wins 非确定，待查）；② MODS −73（z42c 多 73B，逐字节 cmp 定位——疑 native 桩 FUNC/DBUG 编码与 C# 差 + 前序 −33）。

...
- [ ] G17 z42c.ir 逐包对账（进行中，复用 G10-G16 经验；dual-build@0.20 一致工具链下首测）：
  ...
  - [ ] **G17c-3 z42c.ir 末尾残差**（2 项已诊断）：
    ...
    - **② MODS −73（单点结构差，field 类型解析 gap）**：逐字节 cmp 首差 @ MODS off 1079 = `FieldGet`(op 0x60) 结果 tag **z42c=0x20(Object) vs C#=0x04(I32)**——z42c 把某 int 字段读成 Unknown/Object，其后指令流发散且 z42c 长 73B（疑结果 Unknown→下游多发转换/异路径）。根因=z42c typechecker 对该 field（疑跨文件/链式/导入类型）解析失败退 Unknown。需 decode MODS funcData 定位具体 function/field → 独立聚焦迭代。

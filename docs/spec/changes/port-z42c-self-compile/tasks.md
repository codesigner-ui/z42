...
- [ ] G17 z42c.ir 逐包对账（进行中，复用 G10-G16 经验；dual-build@0.20 一致工具链下首测）：
  ...
  - [ ] **G17c-2 z42c.ir 末尾残差**（剩 MODS −73 + 2 串）：① STRS `byte`(z42c) vs `u8[]`(C#)——`Z42PrimType.Name()` 返裸 `byte`，C# canon `byte`→`u8`（`byte[]`→`u8[]`）；② DEPS 串 `z42.core.zpkg`(z42c) vs `z42.cli.zpkg`(C#)（dep provider 分歧，疑 harness 两侧 libs 目录差或 DepIndex first-wins 非确定，待查）；③ MODS −73（z42c 多 73B，逐字节 cmp 定位——疑 native 桩 FUNC 编码与 C# 差 ~40B + 前序 −33）。

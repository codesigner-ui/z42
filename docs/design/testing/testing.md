...
| `src/libraries/<lib>/tests/<name>/` | `.z42` source + expected_output.txt | `z42 xtask.zpkg test vm` + xUnit GoldenTests | 库 API 行为校验（与 [Test] 文件共居）|
| `src/libraries/<lib>/tests/*.z42` | 单文件，含 `[Test]` 注解 | z42-test-runner | 库 [Test] 单元测试 |
| `src/tests/<category>/<name>/` | `.z42` source + expected_output.txt | `z42 xtask.zpkg test vm` + xUnit GoldenTests | VM e2e 按特性分类 |
...
| `src/tests/cross-zpkg/<name>/` | target/ext/main 三 toml 工程 | `z42 xtask.zpkg test cross-zpkg` | 多 zpkg 链接 + 跨包 IR/TSIG 解析 |
...
| `source.zbc` | 可执行测试 | 由 `z42 xtask.zpkg regen` 生成；checked into repo |
...
## 增量测试 `z42 xtask.zpkg test changed`（R3c，2026-04-30）

`z42 xtask.zpkg test changed` 把 `git diff` 的变更文件映射到受影响测试集合，跳过无关测试加快本地反馈。
...
2. 命令行第一参数（`z42 xtask.zpkg test changed main`）
...
| `src/libraries/<lib>/<lib>.toml` | `z42 xtask.zpkg test lib <lib>` |
| `src/runtime/src/**`、`src/runtime/Cargo.toml` | `cargo test runtime` + `z42 xtask.zpkg test vm` |
| `src/runtime/tests/**` | `cargo test --manifest-path src/runtime/Cargo.toml` |
| `src/tests/cross-zpkg/**` | `z42 xtask.zpkg test cross-zpkg` |
| `src/tests/**`（其他） | `z42 xtask.zpkg test vm` |
| `src/compiler/**` | `z42 xtask.zpkg test compiler` + `z42 xtask.zpkg test vm` |
| `src/toolchain/**` | `cargo test test-runner` + `z42 xtask.zpkg test lib` |
| `src/toolchain/xtask` 的 vm / regen 命令 | `z42 xtask.zpkg test vm` |
| `src/toolchain/xtask` 的 lib 命令 | `z42 xtask.zpkg test lib` |
| `src/toolchain/xtask` 的 cross-zpkg 命令 | `z42 xtask.zpkg test cross-zpkg` |
| `scripts/xtask*.z42`、`*.workspace.toml`、`src/runtime/build.rs` | 全套 `z42 xtask.zpkg test` |
| `*.md`、`docs/**`、`.claude/**`、`README*` | 不触发 |
| 其他 `src/**` 或未识别的根级文件 | 全套 `z42 xtask.zpkg test`（防御性） |
...
z42 xtask.zpkg test changed --dry-run
```
...
|------|---------------------------|-----------------|
| 入口 | `<case>/run.sh` exit 0/1 | `cargo bench` / `dotnet run -c Release` / `z42 xtask.zpkg bench` |
...
> `Demo.Inner/Outer/Main`）在 `z42 xtask.zpkg test vm` 的 **jit pass** 下通过，证明
...
R3 落地后通过 `z42 xtask.zpkg test lib z42.text` 运行。
...
```bash
z42 xtask.zpkg build      # dotnet + cargo 全部编译通过
z42 xtask.zpkg test       # compiler + VM + cross-zpkg 全过

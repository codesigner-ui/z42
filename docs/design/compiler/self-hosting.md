...
- **受限/缺口**：runtime builtin 直接 `[Native("__double_to_bits")]` 自声明（免 stdlib 改动）；ctor-less 类合成 `"Class.Class"` ctor token（VM 查无即跳）；char 字面量整链已落地（port-z42c-char）；**异常 try/catch/throw 整链已落地**（port-z42c-try：BoundTry→标签式 lowering[合成 catch-all "*"]→ExceptionTable 编码；trycheck 对账，zbc 5/5）；**DBUG 已落地**（add-z42c-source-spans：AST/Bound 全节点携 Span → TrackLine[每语句,同行去重,file basename] → IrFunction.LineTable/LocalVarTable[RegId 序] → DBUG 第 9 section + HasDebug flag）。

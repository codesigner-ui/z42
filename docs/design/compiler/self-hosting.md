...
- **现状（2026-06-18）= z42c 自身 6/7 包 byte-identical，仅 semantics 剩 2 deep 残差**：dual-build（z42c-self vs C# CLI，绝对 toml 路径消除 DBUG 源路径串差）逐包对账 7 个自身源包 —— **core FULL-FILE 逐字节一致**；`syntax/ir/project/pipeline/driver` 全段 delta 0、disasm **0 非-env diff**，仅剩 STRS −1 = **DEPS provider env-artifact**（同一 namespace 由哪个 stdlib zpkg 提供取决于环境枚举序，`z42.core.zpkg` vs `z42.cli.zpkg` 差 1 字节，非 codegen bug）。**z42c.semantics**：全段 delta 0、disasm 仅剩 **1 处「z42c 更正确」差异**（WithClassGeneric = 确认的 C# bug，不复刻）。已落地的 codegen-parity 根因：① 整型字面量越界 → long（`0xFFFFFFFFL`，`TypeChecker._bindExpr`，镜像 C# `TypeChecker.Exprs` LitIntExpr）；② 无 init 局部 `T m;` 首赋值 → **专用寄存器 + copy**（`_emitAssign`，镜像 C# `WriteBackName`；更正旧「首赋值直接绑寄存器」）；③ cast-to-class receiver 的 field/method 结果 → Unknown，结构化沿 cast→field→call 链传播（`_castUnknownChain`，更正 G18i 过窄跨-ns 门，顺带清掉 ir 的 4 处残差）；④ var-decl-with-init 寄存器 tag = init 表达式类型（非声明类型）；⑤ **STRS 非 ASCII 串按 UTF-8 字节写入**（`ByteWriter.WriteUtf8Bytes`，原 `WriteAsciiBytes` 每 scalar 写 1 字节与 `ByteLength` 错位 → driver banner em-dash 串损坏）；⑥ **重载 arity-mangling**（`TypeChecker._overloadKey` arity-first，修 `mods.Split(" ")`→`Split$1`；根因=`ImportedSymbolLoader` 误注册 bare 别名致发 bare 名）。**唯一剩余差异 = WithClassGeneric（C# bug，不匹配）**：`TypeEnv.Root(s).WithClassGeneric(...)` z42c 正确解析为 `ref`、C# 因方法返回类型解析为无方法 stub class 而发 Unknown（最小复现 `E.Root().With(5)` C# 直接**报错拒绝合法代码**，z42c 正确）。User 裁决「除非原来的有 bug」→ 保留 z42c 正确行为（语义无害：REGT tag 不影响 vcall 按名分派）。

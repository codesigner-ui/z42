...
### z42c 构建 stdlib（replace-csharp-compiler S3：可行性已验证，生产接管**阻塞**）

**S3 目标**：`xtask build stdlib` 的产出端从 C# 换成 z42c（C# 种子 → z42c M2 per-member 重编覆盖 canonical 布局）。**dogfood 验证了可行性，但全量生产接管被两个 z42c parity gap 阻塞，故未落地（`_buildStdlibCore` 仍由 C# 构建）。**
...
**剩余阻塞（S3，3 个**新**z42c codegen bug；8 stdlib test，独立 follow-up）**：full gate 其余**全绿**（C# tests 1571 含 sidecar+multicast / vm goldens 334 / cross-zpkg 2 / 264/272 stdlib）。剩 8 = 3 根因（z42c 编译 z42c-built stdlib 行为偏差，C#-built 正常）：
- **cross-namespace 静态调用**（4× ZipWriteTests）：`Std.Archive.Zip.Write` 调 `Deflate.Compress` → z42c 误限定为 `Std.Archive.Deflate.Compress$1`（当前 ns）而非 imported `Std.Compression.Deflate`，运行时 undefined。
- **blake3 多块 z42c codegen**（1× 我的 blake3 回归测试）：z42c-built `Blake3.HashHex(2049B)` ≠ C#-built（源 fix 正确，C#-built 过；z42c **miscompiles** 多块树代码某构造）。
- **diagnostics min-level / color**（3× DiagnosticsFilter/LogFormat）：z42c-built 行为偏差（min-level gate / ANSI），疑 enum / 静态字段 codegen。

> **S3 落地前置**：解上述 3 个 z42c codegen bug（各独立 follow-up）。就位后翻转 `_buildStdlibCore` 重跑 full gate。perf 税（z42c interp 重编 ~30s）见 Deferred `self-hosting-future-z42c-stdlib-jit`。
...
- **触发原因**：S3（z42c 接管 stdlib build，**当前阻塞未落地**）若落地，`_buildStdlibCore` 用 z42c interp 重编 stdlib ~30s 加到每次 `build stdlib`（dogfood 税）。
- **前置依赖**：先解 S3 两个阻塞（BLID sidecar + multicast aggregate）；再 z42c `--mode jit` 实测 22 库 jit-built == interp-built 功能等价（fix-jit-cross-zpkg-call 已修 JIT 跨包，理论可行）。
- **触发条件**：S3 落地后、stdlib 构建成迭代瓶颈时。
- **当前 workaround**：S3 未落地，生产 stdlib 仍 C# 构建（快）。

### self-hosting-future-s3-remaining-codegen-bugs

- **来源**：dogfood S3（dogfood-z42c-stdlib-build，2026-06-22）
- **触发原因**：z42c 接管生产 stdlib build 后，full gate 余 8 stdlib test 失败 = **3 个 z42c codegen bug**（z42c-built 行为偏 C#-built）：① cross-namespace 静态调用误限定当前 ns（Zip→Deflate，4 test）；② blake3 多块 z42c codegen（z42c miscompiles 多块树代码，1 test）；③ diagnostics min-level/color codegen（3 test）。compression `[Native]` 已修（fix-z42c-native-named-entry）。
- **前置依赖**：逐个排查/修复 3 个 z42c codegen 根因（各独立 change）。
- **触发条件**：S3（z42c 接管 stdlib build）落地——3 bug 全解后翻转 `_buildStdlibCore`。
- **当前 workaround**：S3 未落地，生产 stdlib 仍 C# 构建。已修 5 bug：TSIG / generic-ctor / strip-symbols / blake3 多块 / native named-entry。

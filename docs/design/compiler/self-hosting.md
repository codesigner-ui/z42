...
- **z42c-built z42.compression 的 `[Native]` extern 解析**：z42c-built（strip）`z42.compression.zpkg` 在测试时报 `VM error: undefined function Std.Compression.Brotli._CompressRaw`（18 个 compression test）。native 绑定串（`__brotli_compress` / `entry`）在 zpkg STRS 中存在，但 VM 不识别 `_CompressRaw` 为可调用 native——疑 z42c 在 strip 路径下 `[Native(lib,entry)]` 属性与函数的**绑定**（SIGS attr-ref / FUNC 元数据）未正确 emit，或 cdylib 路径解析。**非 strip 的 z42c-built compression 此前 272 全过**，故疑 strip 相关。需独立排查。其余全绿（C# tests 含 sidecar+multicast、vm goldens、cross-zpkg、254/272 stdlib）。

> **S3 落地前置（仅剩一项）**：解 z42c-built z42.compression `_CompressRaw` 解析。就位后翻转 `_buildStdlibCore` 重跑 full gate。perf 税（z42c interp 重编 ~30s）见 Deferred `self-hosting-future-z42c-stdlib-jit`。
...
### self-hosting-future-z42c-compression-native-binding

- **来源**：dogfood S3（dogfood-z42c-stdlib-build，2026-06-22）
- **触发原因**：z42c-built（strip）`z42.compression.zpkg` 运行时 `_CompressRaw` 等 `[Native(lib,entry)]` extern 报 `undefined function`（18 compression test）。native 绑定串在 zpkg STRS 中，但 VM 不识别为可调用 native。
- **前置依赖**：排查 z42c strip 路径下 `[Native]` 属性与函数绑定（SIGS attr-ref / FUNC 元数据）emit 是否正确，或 cdylib（`z42_compression`）路径解析；对比 z42c-built non-strip（272 全过）与 strip 的差异。
- **触发条件**：S3（z42c 接管 stdlib build）落地——这是其唯一剩余阻塞。
- **当前 workaround**：S3 未落地，生产 stdlib 仍 C# 构建（compression 正常）。

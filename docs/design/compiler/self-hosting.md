...
- **现状（2026-06-18）= z42c 自身 4 个源包 byte-identical**：dual-build（z42c-self vs C# CLI，绝对 toml 路径消除 DBUG 源路径串差）对账 `z42c.{core,syntax,ir,project}` —— **core FULL-FILE 逐字节一致**；`syntax/ir/project` 全段 delta 0，仅剩 STRS −1 = **DEPS provider env-artifact**（同一 namespace 由哪个 stdlib zpkg 提供取决于环境枚举序，`z42.core.zpkg` vs `z42.cli.zpkg` 差 1 字节，非 codegen bug）。最后一处 codegen 残差（`z42c.project` 的 `0xFFFFFFFFL` mask 寄存器 REGT i32-vs-i64）由**整型字面量越界类型推断**修复：值 > int.MaxValue（或 < int.MinValue）→ 类型 long，否则 int（`TypeChecker._bindExpr` IntLitExpr 分支，值解析复用 `ZbcInstr._parseIntLit`，镜像 C# `TypeChecker.Exprs` LitIntExpr 规则）。**剩 3 包 = z42c.{semantics,pipeline,driver}**。

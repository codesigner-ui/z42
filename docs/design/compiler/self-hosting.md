...
- **受限/缺口**：runtime builtin 直接 `[Native("__double_to_bits")]` 自声明（免 stdlib 改动）；ctor-less 类合成 `"Class.Class"` ctor token（VM 查无即跳）；char 字面量待前端整链；**DBUG 是全面 byte-identical 的唯一阻塞**（C# 对有语句体函数 emit 源码行表；z42c AST 无 span → 独立 change `add-z42c-source-spans`）。
...
- **现状**：`.zbc` 写入器功能完整 + `empty` 逐字节绿；全函数逐字节对账待 span→LineTable→DBUG 链（`add-z42c-source-spans`）。

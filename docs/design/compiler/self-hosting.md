...
**剩余阻塞（S3，2 个**新** z42c codegen bug；4 stdlib test，独立 follow-up，self-hosting-future-s3-remaining-codegen-bugs）**：full gate 其余**全绿**（C# 1571 含 sidecar+multicast / vm goldens 334 / cross-zpkg 2 / ~268/272 stdlib）：
- **blake3 多块 z42c codegen**（1×）：z42c-built `Blake3.HashHex(2049B)` ≠ C#-built（源 fix 正确，C#-built 过；z42c **miscompiles** 多块树代码——疑 `_reduceParents` 循环里 `cvList = next`（重赋参数数组）或 `(p/2)*8` 索引）。
- **静态字段 mutation 不持久**（3× DiagnosticsFilter/LogFormat）：`Log.SetMinLevel(3)` 后 `GetMinLevel()` 仍返默认 2——z42c-built 静态字段**写**不持久（疑 `__static_init__` 每次重跑 OR 静态字段写入/读取未共享）。z42c 自身包静态字段多为只读（sacheck 等过读取），mutation 路径未测到。

> **S3 落地前置**：解上述 2 个 z42c codegen bug（各独立 follow-up）。就位后翻转 `_buildStdlibCore` 重跑 full gate。perf 税见 Deferred `self-hosting-future-z42c-stdlib-jit`。
...
- **触发原因**：z42c 接管生产 stdlib build 后，full gate 余 **2 个 z42c codegen bug**（4 stdlib test；z42c-built 行为偏 C#-built）：① blake3 多块 z42c codegen（z42c miscompiles 多块树代码，1 test）；② 静态字段 mutation 不持久（diagnostics min-level/color，3 test）。
- **前置依赖**：逐个排查/修复 2 个 z42c codegen 根因（各独立 change）。
- **触发条件**：S3 落地——2 bug 全解后翻转 `_buildStdlibCore`。
- **当前 workaround**：S3 未落地，生产 stdlib 仍 C# 构建。已修 6 bug：TSIG / generic-ctor / strip-symbols / blake3 多块 / native named-entry / cross-ns 静态调用。

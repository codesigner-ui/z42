...
| `z42.threading` | L2 | Thread / ThreadException + Mutex<T> / Channel<T> / ChannelDisconnectedException：OS 线程 spawn / join + 同步原语（RAII Mutex callback + unbounded MPSC channel） | ✅ VM intrinsic（`__thread_*` / `__mutex_*` / `__channel_*`） |

...
| `z42.threading` | L2 | Thread / ThreadException + Mutex<T> / Channel<T> (unbounded + `new Channel<T>(N)` bounded with back-pressure) / ChannelDisconnectedException：OS 线程 spawn / join + 同步原语 | ✅ VM intrinsic（`__thread_*` / `__mutex_*` / `__channel_*`） |

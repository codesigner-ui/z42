...
| ~~`net-future-socket-options`~~ (Nagle / IP_TTL / SO_REUSEADDR / SO_KEEPALIVE 全套落地) | **✅ 已落地 2026-05-30** (`add-net-socket-options-extended` 完成 SO_REUSEADDR + SO_KEEPALIVE)；Nagle / IP_TTL 已 5-27 落地. SO_KEEPALIVE bool-only; tuning (idle/interval/probes) 留 `net-future-keepalive-tuning` follow-up. SO_REUSEADDR 仅 pre-Bind 合法（post-Bind 抛 InvalidOperationException, 对齐 BCL ExclusiveAddressUse 语义） | — |
...
| ~~`bigint-future-modpow`~~ + ~~`bigint-future-modpow-negexp`~~ + ~~`bigint-future-modinverse`~~ + ~~`bigint-future-prime`~~ + ~~`bigint-future-prime-sieve`~~ + ~~`bigint-future-modpow-montgomery`~~ + ~~`bigint-future-prime-deterministic`~~ | **✅ ModPow / ModInverse / IsProbablyPrime / NextPrime / Miller-Rabin + small-prime sieve + Montgomery REDC fast path 全部已落地 2026-05-25 → 2026-05-28；deterministic `IsPrime()` 已落地 2026-06-02** — RSA / ECDH / Ed25519 / ECDSA primitives all auto-use Montgomery when modulus is odd; `IsPrime()` tiered Miller-Rabin per OEIS A014233 for `n < 3.317×10^24`; `bigint-future-bpsw` 仍延后 | — |

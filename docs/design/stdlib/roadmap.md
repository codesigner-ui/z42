...
| ~~`bigint-future-modpow`~~ + ~~`bigint-future-modpow-negexp`~~ + ~~`bigint-future-modinverse`~~ + ~~`bigint-future-prime`~~ + ~~`bigint-future-prime-sieve`~~ + ~~`bigint-future-modpow-montgomery`~~ | **✅ ModPow / ModInverse / IsProbablyPrime / NextPrime / Miller-Rabin + small-prime sieve + Montgomery REDC fast path 全部已落地 2026-05-25 → 2026-05-28** — RSA / ECDH / Ed25519 / ECDSA primitives all auto-use Montgomery when modulus is odd; deterministic small-bound primality (`bigint-future-prime-deterministic` / `bigint-future-bpsw`) 仍延后 | — |

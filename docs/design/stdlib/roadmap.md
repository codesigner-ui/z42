...
| **z42.crypto** | `System.Security.Cryptography` | `ring` / `sha2` / `aes` | L2 | FFI | ✅ SHA-1 / SHA-256 / SHA-384 / SHA-512 / SHA3-224/256/384/512 + SHAKE128/256 + Keccak-256/512 (legacy) + BLAKE2b + BLAKE2s + HMAC for SHA-1/256/384/512 + CSPRNG `SecureRandom` + PBKDF2 + HKDF-SHA-256/512 + scrypt + AES-128/192/256 (ECB + CTR + CBC-PKCS7 + GCM) + ChaCha20 + Poly1305 + ChaCha20-Poly1305 AEAD + X25519 ECDH + Ed25519 signing + RSA (PKCS#1 v1.5 + PSS sign/verify + OAEP encrypt/decrypt + raw RSAEP/RSADP) + ECDSA P-256 (RFC 6979 deterministic) 已落地 (2026-05-24 → 2026-05-28)；hw-accel / non-96-bit-IV-GCM / Argon2 / BLAKE3 / ECDSA secp256k1 留 follow-up |
...
| `aes-future-gcm-non96iv` | AES-GCM with non-96-bit IV (GHASH-derived J0) | rare; spec compliance |
...
| ~~`bigint-future-modpow`~~ + ~~`bigint-future-modpow-negexp`~~ + ~~`bigint-future-modinverse`~~ + ~~`bigint-future-prime`~~ + ~~`bigint-future-prime-sieve`~~ | **✅ ModPow / ModInverse / IsProbablyPrime / NextPrime / Miller-Rabin + small-prime sieve 全部已落地 2026-05-25** — covers RSA / ECDH primitive needs; Montgomery-form fast path (`bigint-future-modpow-montgomery`) + deterministic small-bound primality (`bigint-future-prime-deterministic` / `bigint-future-bpsw`) 仍延后 | — |

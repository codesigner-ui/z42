...
- `z42.net` K4（2026-05-25 add-z42-net-websocket）— `Std.Net.WebSockets.{WebSocketClient, WebSocketMessage, WebSocketMessageType, WebSocketState, ...}` RFC 6455 client over K1 TCP. 纯脚本，无新 VM builtin. ws:// only — wss:// 等 `add-z42-net-tls`. Handshake + single-frame send + multi-frame receive assembly + auto-pong + close handshake. Sec-WebSocket-Accept validation skipped v0 (z42.crypto 缺 SHA-1)。HTTP/2 / async 仍是 follow-up.
...
| `net-future-udp-connected` | `UdpClient.Connect(host, port)` + bare Send/Receive | K2 v0 always-explicit-addr; follow-up |
| `net-future-udp-multicast` | `JoinMulticastGroup` + multicast send | multicast use case 驱动 |
| `net-future-udp-recv-into` | buffer-fill `Receive(byte[])` 变体 | bench 显示 per-call alloc 是瓶颈 |
...
| `net-future-http-redirects` | auto-follow 3xx | OAuth / login 流程 |
| `net-future-http-cookies` | Set-Cookie 解析 + cookie jar | session-based API 用例 |
| `net-future-http-auth` | Basic / Bearer / Digest 辅助 | 简化 auth header 处理 |
| `net-future-http-compression` | Accept-Encoding + 自动 gzip/br decode | bandwidth-sensitive 用例 |
| `net-future-http-server` | `HttpListener` / `HttpServer` | 服务端 / API mock 用例 |
| `net-future-http2` | HTTP/2 binary framing + HPACK | gRPC 等高吞吐用例 |
| ~~`net-future-websocket`~~ | ~~WebSocket protocol (RFC 6455) over K1 TCP~~ | **✅ 已落地 2026-05-25 (add-z42-net-websocket K4)** — ws:// only; wss:// 等 `net-future-tls`; SHA-1 验证 Accept 等 `add-z42-net-websocket-accept-validate` |
| `net-future-websocket-accept-validate` | Sec-WebSocket-Accept SHA-1 verification client-side | z42.crypto Sha1 落地 |
| `net-future-websocket-server` | server-side WebSocketListener | server use case |
...
| ID | 标题 | 前置依赖 |
|----|------|---------|
| `bigint-future-bitops` | AND / OR / XOR / NOT / shift on BigInt | crypto / bitset 用例驱动 |
| `bigint-future-modpow` | `ModPow(exp, modulus)` modular exponentiation | z42.crypto RSA / ECDH |
| `bigint-future-gcd` | Gcd / Lcm / IsProbablyPrime / NextPrime | RSA key gen / 数论 |
...
| `numerics-future-complex` | `Std.Numerics.Complex` 复数 | 信号处理用例 |
| `numerics-future-decimal` | `Std.Numerics.Decimal` 定点小数 | 金融用例 |

...
| ID | 标题 | 前置依赖 |
|----|------|---------|
| `io-stream-future-filestream` | `FileStream : Stream` 包 Std.IO.File primitive | 大文件流式读用例 |
| `io-stream-future-textreader` | `TextReader / TextWriter`（编码 + 行处理） | 行式 IO 用例 |
| `io-stream-future-bufferedstream` | `BufferedStream` wrapper | 小 IO 性能瓶颈 |
| `io-stream-future-async` | `ReadAsync` / `WriteAsync` | L3 async/await |
| `io-stream-future-copy-to` | `Stream.CopyTo(dest)` 便捷 | 用例驱动 |
| `refactor-compression-stream-on-iostream` | `CompressionStream` extends Stream + `Gzip.WrapStream` | pipeline 组合用例（z42.net 落地附带）|
| `refactor-binary-reader-stream` | `BinaryReader/Writer` 接受 Stream（非仅 byte[]） | 大文件结构化读用例 |

...
- `z42.net` K2（2026-05-25 add-z42-net-udp）— `Std.Net.Sockets.{UdpClient, UdpReceiveResult}` 同步阻塞 UDP。Auto-bind on first Send；ephemeral port discovery via `LocalPort()`. 复用 K1 的 kind-tuple shape + slot-table pattern。IPAddress / DNS / Timeout / TLS / HTTP / WebSocket / async 仍是 follow-up spec.

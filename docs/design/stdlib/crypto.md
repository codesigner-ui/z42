...
- AES (FIPS 197) — `Std.Crypto.Aes` (add-aes, 2026-05-27)
  ...
  - `EncryptCcm(byte[] key, byte[7..13] nonce, byte[] aad, byte[] plaintext, int tagLen) -> byte[]` / `DecryptCcm` — AEAD per RFC 3610 / NIST SP 800-38C. CTR + CBC-MAC construction (no GHASH — simpler than GCM, popular for IoT). nonce length determines max payload (2^(8·(15-nonceLen)) - 1 bytes); tagLen in {4,6,8,10,12,14,16}. Used in WPA2-CCMP, Bluetooth LE Secure Connections, Zigbee, IEEE 802.15.4, TLS_PSK_WITH_AES_128_CCM_8

...
> 本文与下列规范的关系：
>
> - [interop.md](interop.md) 解决 **native 代码 → 注册类型/方法进 z42**（"扩展语言"）。本文解决 **宿主 app → 启动并驱动 VM**（"嵌入运行时"）。两者复用同一份 `Z42Value` / `Z42Args` 类型，互不重叠。
> - [cross-platform.md](cross-platform.md) 决定 VM **如何编译**到 ios/android/wasm。本文决定编译产物**如何被宿主调用**。
> - [cross-platform-testing.md](cross-platform-testing.md) 的 test-runner 是本 API 的**首要消费者**之一；MVP 之后 runner 将基于本 API 重构（见 §12 Deferred）。
...
2. **三层 ABI**（与 [interop.md §2](interop.md) 同构） — Tier 1 稳定 C ABI；Tier 2 Rust 人因工程；Tier 3 平台 facade（Swift / Kotlin / JS）。
3. **AOT 友好** — 入口解析按 FQN（fully qualified name）字符串查找，运行时不依赖反射元数据生成器。iOS 禁 JIT 场景下走 interp 或 AOT。
4. **零拷贝优先** — 标量值通过 `Z42Value` 直接传递；`String` / `Array<T>` 通过 `pinned` 块跨边界（沿用 [interop.md §6.3](interop.md)）。
...
| Milestone | 内容 | 依赖 |
|-----------|------|------|
| **H0 spec** | 本文档 + spec/changes/add-embedding-api/ DRAFT | — |
...
> 本节遵循 [feedback_deferral_location.md] 约定：设计期延后写在本节，不进 docs/deferred.md。

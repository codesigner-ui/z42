...
### attr-factory-qualified-base-upcast — 限定基名的 attribute 无法 upcast（compiler bug）

- **来源**：fix-reflection-test-compile（2026-06-10，add-field-attribute-reflection 的 reflection.z42 编译失败暴露）。
- **现象**：attribute 类基名写成**限定 FQN**（`class FieldTag : Std.Attribute`）时，`AttributeFactorySynthesizer` 合成的 factory `public Attribute __f() { return new FieldTag(...); }` 在 `RequireAssignable`（[TypeChecker.Stmts.cs:421](../../../src/compiler/z42.Semantics/TypeCheck/TypeChecker.Stmts.cs)）报 `return type mismatch: expected Attribute, got FieldTag` —— upcast 检查未规范化 FQN，`Std.Attribute`（派生基）与 factory 的 unqualified `Attribute` 返回类型被当成不同类型。unqualified `: Attribute`（via `using Std`）正常（三个 golden `src/tests/attributes/*.z42` 皆如此）。
- **根因**：typechecker 的 assignability/base-chain 比较按名而非 canonical FQN。与 archived `fix-fqn-class-resolution`（2026-06-09）同族。
- **当前 workaround**：attribute 类基名写 unqualified `: Attribute`（`using Std` 在作用域内）。
- **前置依赖**：根因修复属 compiler 改动，与进行中的 `port-z42c-*` 自举移植可能冲突 → 独立 change。
- **触发条件**：自举 port 收口后，或用户实际写 `: Std.Attribute` 受阻时。

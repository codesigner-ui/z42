...
- **剩余**：jagged 数组类型 `int[][]` 的 `typeof` 语法暂不支持（type 解析器不接受嵌套 `[]`）；多维数组同。z42c 自举 writer 同步 **已落地**（2026-06-16 port-z42c-self-compile：`ArrayNew` element_type FQ 名已镜像，z42c.core FULL-FILE byte-identical；后续 zbc 1.17/1.18 版本号同步随之跟进）——`xtask test compiler-z42` byte-identical gate 恢复全绿。

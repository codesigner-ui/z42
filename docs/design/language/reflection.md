...
### reflection-future-typeof-returns-type
- **来源**：add-reflection-mvp
- **触发原因**：`typeof(T)` 当前返回**字符串**（类型名），非 `Std.Type`。改为返回 Type 需动 C# 编译器 IrGen/TypeChecker，与正在进行的 `port-z42c-*` 自举移植需协调。
- **前置依赖**：与 B 自举流协调编译器改动。
- **触发条件**：用户需要 `typeof(int).GetFields()` 式编译期类型反射时。
- **当前 workaround**：`obj.GetType()` 路线全可用。

...
| 类型 | 命名空间 | 成员 |
|------|---------|------|
| `Type` | `Std`（z42.core）| `Name` / `FullName` / `BaseType` / `IsAbstract` / `IsSealed` / `IsValueType` / `IsRecord` / `GetFields()` / `GetMethods()` / `GetMembers()` / `GetProperties()` / `GetGenericArguments()` |
...
- **实现尝试（2026-06-10，3 次均失败，逐层揭深）**：确认这是「过深」坑（同 ACTIVE.md `attr-factory-return-type-resolution` 的判断）。
  1. **只修 P2**（ResolveTypeName 含 `.` 名字剥命名空间重查短名）→ 编译过、零回归（244 golden），但链式仍崩 —— 因为崩在 P1（`f.GetType()` 是 Unknown，第一跳就降级）。
  2. **P1 在 MergeImported 内 adopt 真 GetType** → 不触发：`MergeImported`(Collect L70) 在 `CollectClasses`(L75) **之前**跑，stub 此时还没建；且 import key 短/FQN 双键（stub `"Object"` vs import 可能 `"Std.Object"`）让 adopt 条件错配。
  3. **P1 移到 `CollectClasses` 之后直接 fixup stub GetType 返回类型**（查 `_classes["Type"]`/`["Std.Type"]` 鲁棒取 Std.Type）→ **仍不生效**，链式仍崩在第一跳（即便加 `using Std`）。说明还有第 4 层：fixup 时 `_classes` 里 Std.Type 不以 `"Type"`/`"Std.Type"` 为 key（可能 mangle / 或 GetType 的 bound 类型来自另一条解析路径 / 或存在多次 SymbolCollector.Collect 各自的 `_classes`）。
- **下次接手起点**：先加临时 `Console.Error` 打印 FixupObjectGetTypeReturn 是否命中 + `_classes` 的实际 keys，定位第 4 层（Std.Type 的真实 key / GetType bound 类型来源），再动手。standalone `--dump-bound` 不可信（见上）。
- **触发条件**：两 part（实为 ≥4 层）一起修 + 全测量 blast radius（Members.cs:174 同族 load-bearing 区，须测回归）；与 `port-z42c-*` 协调（改 codegen/typecheck 动 byte-identical，但仅影响用链式反射的程序，自举源码罕用）。

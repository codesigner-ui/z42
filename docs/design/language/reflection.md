...
> **2026-06-16 add-reflection-assignable-from（zbc 1.20）**：接口块从 bare 名（`IShape`）改存 **FQ 名**（`Demo.IShape`，编译期 `QualifyClassName`）。因接口现产 TYPE 条目（add-reflection-interface-class-predicates），`make_type_from_name(FQ)` 命中接口 TypeDesc → `GetInterfaces()` 返回**真接口句柄**（`IsInterface`/`FullName` 可用，不再 name-only）；接口身份按 FQ 名 robust 比较（修跨命名空间同名歧义）。**传递接口实现**（`C : IList`、`IList : ICollection` → 含 `ICollection`）仍延后：InterfaceDecl 不携带 base-interface 列表，拿不到接口继承图（见 Deferred）。
...
- **触发原因**：`C : IList`、`IList : ICollection` → `GetInterfaces()` / `IsAssignableFrom` / `is` 应含 `ICollection`（传递闭包）。当前只做"类直接声明 + 类继承链聚合"，不展开接口的 base-interface。
- **前置依赖**：接口的 base-interface 列表需持久化——`InterfaceDecl` 当前不携带（接口 TYPE 条目是最小条目，无接口块）。
- **触发条件**：需要完整 C# 传递接口语义时。

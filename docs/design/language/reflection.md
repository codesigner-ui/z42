...
| 类型 | 命名空间 | 成员 |
|------|---------|------|
| `Type : MemberInfo` | `Std`（z42.core）| `Name`（继承自 MemberInfo）/ `FullName` / `BaseType` / `IsAbstract` / `IsSealed` / `IsValueType` / `IsRecord` / `GetFields()` / `GetMethods()` / `GetMembers()` / `GetProperties()` / `GetGenericArguments()` / `GetInterfaces()` |
...
**设计选型**：纯运行期派生（不持久化任何新元数据），延续 properties / inherited-static-fields / value-record-flags 的"零格式成本"模式——比需 wire bump 的 type-flags / static-fields 增量更轻。`IsGenericTypeDefinition`（开放定义 vs 实例化）收窄延后：z42 `typeof(Box<int>)` 当前解析到定义 `TypeDesc`（编译器丢实例化 type args），运行期无法区分开放/封闭泛型——见 Deferred。
...
- **剩余**：jagged 数组类型 `int[][]` 的 `typeof` 语法暂不支持（type 解析器不接受嵌套 `[]`）；多维数组同。z42c 自举 writer 同步 **延后**（z42c 子系统被 `port-z42c-package-symbols` 占用）——`xtask test compiler-z42` byte-identical gate 暂红，待自举 port 归还锁后补 `ArrayNew`/`ArrayNewLit` 的 element_type 字段。
...
### reflection-future-generic-type-definition
- **来源**：add-reflection-generic-predicates design.md Decision 2
- **触发原因**：`Type.IsGenericTypeDefinition`（开放泛型定义 `Box<>` vs 实例化 `Box<int>`）与 `Type.GetGenericTypeDefinition()` 尚不可表达——`typeof(Box<int>)` 当前解析到定义 `TypeDesc`，编译器丢弃实例化 type args，运行期开放/封闭不可区分。
- **前置依赖**：`typeof` 携带实例化 type args（编译期保留 + wire 持久化），或 0.5.x generic instantiation。
- **触发条件**：需要区分开放/封闭泛型，或 `MakeGenericType` 落地时。

### reflection-future-type-category-flags
- **来源**：add-reflection-generic-predicates tasks.md 收窄
- **触发原因**：`Type.IsClass` / `IsInterface` / `IsEnum` 需要类别元数据——当前 TYPE section flags 字节只捕获 abstract/sealed/struct/record，无 class-vs-interface-vs-enum 区分位。
- **前置依赖**：TYPE section flags 扩位（format bump），编译期写入类别。
- **触发条件**：需要按类别筛选反射类型时（小增量，沿 type-flags 已验证的 flags-bit 模式）。

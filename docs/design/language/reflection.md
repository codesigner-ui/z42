...
### ~~reflection-future-attributes（C3）~~ — class-level 已落地
- **状态**：用户自定义 attribute + 反射（class-level）已落地 2026-06-09（C3a，archive/2026-06-09-add-attribute-reflection）。`class Foo : Attribute` + `[Foo(args)]` 标注 class → `Type.GetCustomAttributes()` / `GetAttribute(Type)` 返活实例。设计 + 实现原理见 [attributes.md](attributes.md)。
- **剩余（C3b）**：method-level（`[Doc] void M()` → `MethodInfo.GetCustomAttributes()`）；见 attributes.md Deferred `attribute-future-method-level`。

...
| 平台 | 默认 resolver | 备注 |
|------|--------------|------|
| iOS | `BundleZpkgResolver()` 读 `Bundle.main.url(forResource: "<ns>", withExtension: "zpkg", subdirectory: "stdlib")` | facade `Z42VM` 构造时自动装 |
| Android | `AssetZpkgResolver(context.assets)` 读 `assets/stdlib/<ns>.zpkg` | facade `Z42VM` 构造时需传 `Context` |
| WASM | `MapZpkgResolver(Map<string, Uint8Array>)` 由 JS 预 fetch 后传入；或 `JsCallbackResolver(fn)` 同步回调 | wasm-bindgen 包装层 |

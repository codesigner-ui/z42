...
| 平台 | 默认 resolver | 备注 |
|------|--------------|------|
| iOS | `BundleZpkgResolver(bundle: .main, subdirectory: "stdlib")` 先查 `stdlib/index.json` 把 namespace map 到 zpkg 文件名，再读 `Bundle.url(forResource:withExtension:subdirectory:)`；index 缺失或 miss 时回退到 `<ns>.zpkg` | facade `Z42VM` 构造时自动装 |
| Android | `AssetZpkgResolver(context.assets, "stdlib")` 同行为：先查 `assets/stdlib/index.json`，再 `assets/stdlib/<ns>.zpkg` fallback | facade `Z42VM` 构造时需传 `Context` |
| WASM | `bundleStdlibBrowser(stdlibUrl)` / `bundleStdlibNode()` —— 内部先 fetch / readFile `stdlib/index.json`，再按映射读对应 zpkg，最后构 `mapResolver(Map<string, Uint8Array>)`；index 缺失时回退到 namespace-as-filename。自定义 host 也可直接传 `(name) => Uint8Array` 函数 / `{ resolve(name) }` 对象。| wasm-bindgen 包装层 |

### 11.7.1 Namespace index（mobile / wasm 默认 resolver 用）

一份 zpkg 通常提供**多个** namespace（如 `z42.core.zpkg` 同时 ship `Std` 与 `Std.Exceptions`），不能假设 `namespace == 文件名`。Mobile / wasm 默认 resolver 通过 stdlib 目录中的 `index.json` 把 namespace 解析为具体 zpkg 文件名：
...
- **生成**：host `z42 xtask.zpkg build stdlib` 在产 flat view `artifacts/build/libraries/dist/<profile>/` 时同步写 `index.json`
- **分发**：iOS / Android `build.sh` 把 `index.json` 与 zpkg 一起拷进 bundle / asset 目录
- **加载**：resolver 构造时 eager 读 index、建 `Dictionary<String, String>` 缓存；`resolve()` 优先按 index 查文件名
- **Fallback**：index 缺失 / namespace 不在 index 时回退到 namespace-as-filename（向后兼容自定义 resolver 摆放方式）
- **自定义 resolver**：可不读 index，自行维护 namespace → bytes 映射（如 wasm 端 `JsCallbackResolver`）

Spec：[`docs/spec/archive/2026-05-12-fix-bundle-resolver-namespace-index/`](../../spec/archive/2026-05-12-fix-bundle-resolver-namespace-index/)

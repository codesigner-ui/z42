...
② $Z42_HOME/launcher  已安装根（probe <root>/z42vm 或 <root>/bin/z42vm + <root>/libs）。
③ exe 上行 .z42       逐级 <d>/.z42/launcher、<d>/.z42 —— framework-dependent app（xtask 这类）的项目 venv。
④ 系统 $HOME/.z42/launcher
...
**两个旋钮**：`$Z42_PORTABLE_VM`（①）显式钉死任意 vm（CI/镜像/特殊部署）或承载 SDK 同址 vm，最高优先；`$Z42_HOME`（②）改"系统安装根"位置。

...
native apphost stub（`src/toolchain/launcher/src/apphost.rs`，共享 `lib.rs`；**统一后 `z42` 自身也是此 stub，原 trampoline 实现移除** —— 待 code change）内嵌一段固定占位区：32 字节 MAGIC sentinel + 992 字节 payload（共 1024）。`z42 publish desktop`（z42 写，`core/apphost.z42`）：
...
### 运行时解析：z42vm 探测三档（统一 apphost 唯一真相，2026-06-20）

统一 trampoline → apphost 后，**所有原生入口共用同一个 stub、同一套 `probe_app_runtime`**：`z42`（launcher，包根）、`bin/` 内工具 apphost（`z42c` 自举后及后续）、per-app apphost（`./xtask` 等）。stub 不捆绑 VM/libs（framework-dependent），按**三档**定位 **`z42vm`**（设 `Z42_LIBS`；**不需 `launcher.zpkg`**）：

```
① 自定义覆写   $Z42_PORTABLE_VM                     显式指向 z42vm（文件或其目录）；CI/镜像/特殊部署绕过约定
② exe 近邻     （锚定 current_exe 所在目录，非 CWD）—— 推 install root 找 <root>/bin/z42vm：
   a. {exe_dir}/z42vm        exe 在 bin/ 内，z42vm 是同级兄弟 —— z42c 及后续 bin/ 内 apphost
   b. {exe_dir}/bin/z42vm    exe 在包根 —— launcher z42、self-contained app
   c. 逐级上行 <d>/.z42/bin/z42vm    项目 venv —— xtask 这类 framework-dependent app（.z42 与 exe 并列或在上层）
③ 系统安装     $Z42_HOME/bin/z42vm（未设 $Z42_HOME → 默认 $HOME/.z42/bin/z42vm）
   都无 → 报错列已查路径，非零退出
```

**a/b 两形态 = 两种 install root 推法**：apphost 可能装在包根（`z42`）也可能装在 `bin/` 里（`z42c` 及后续工具），二者的 z42vm 都在 **`<root>/bin/z42vm`**——从 exe 反推 root：exe 在 bin/ 内则 root = 父目录（vm 即同级 a）；exe 在根则 root = exe_dir（vm 在子 bin/ 即 b）。两者最终都指向同一个 `$Z42_HOME/bin/z42vm`。

**colocated（a/b）必须在 venv 上行（c）之前是正确性硬要求**：每个 apphost 的 payload 永远按 exe 目录解析 ⇒ 来自**自己那个包**，与同包 `bin/z42vm` 同版本（zbc ABI 必配）。所以必须先命中 colocated vm，**绝不能落到项目本地（c）/ 系统（③）的异版本 vm**（否则新版 zpkg 在旧版 vm 上加载 → zbc 不兼容炸）。

**一套顺序服务全部角色**——靠"exe 近邻有没有 colocated vm"自然分流：
- **launcher `z42`**（包根，子 `bin/z42vm`）→ ②b 命中 ✓
- **`bin/z42c` 及后续 bin/ apphost**（与 z42vm 同在 `bin/`）→ ②a 命中同级 vm，**直接跑、不经 launcher** ✓
- **per-app `./xtask`（framework-dependent）**（exe 近邻无 colocated vm）→ ②c 命中项目 `.z42`，**venv 语义不变** ✓
- **per-app self-contained（捆 vm，future）** → a/b 命中捆绑 vm ✓

**bin/ 内 apphost 独立运行**：`z42c`（自举后）/ 未来工具是 apphost，payload 指向**自身 zpkg**（如 `bin/z42c` → `../programs/z42c/z42c.driver.zpkg`），经 ②a 找到同级 z42vm，**`exec z42vm <自身zpkg>` 直接跑，不路由经 `launcher.zpkg`**。其 `Z42_LIBS = <root>/libs + dirname(payload)`（如 z42c = `libs/ + programs/z42c/`，自动带上自身 zpkg 同目录的依赖）。

**两个旋钮**：`$Z42_PORTABLE_VM`（①）显式钉死任意 vm，最高优先；`$Z42_HOME`（③）改"系统安装根"位置。② 全是约定路径、无需配置——**绝大多数情况只走 ②**。

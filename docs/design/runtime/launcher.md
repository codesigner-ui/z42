...
`z42 xtask.zpkg package` 的 desktop 包内置 launcher,**解压即用、无需安装**:
...
包根的 `install.sh`(由 `z42 xtask.zpkg package` 的 desktop 路径内置)把包铺进 `$Z42_HOME`(默认 `~/.z42`):
...
- `z42 xtask.zpkg package` → `<pkg>/bin/apphost`（patcher 便携模式从 `dirname(Z42_PORTABLE_VM)/apphost` 取）。
...
- **前置依赖**：每平台×版本的 `z42vm` 发布点（GitHub Releases；`z42 xtask.zpkg package` 产 host-RID 包可作产物）+ 完整性校验

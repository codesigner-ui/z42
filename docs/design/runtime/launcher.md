...
native apphost stub（`src/toolchain/launcher/src/apphost.rs`，与 trampoline 同 crate、共享 `lib.rs`）内嵌一段固定占位区：32 字节 MAGIC sentinel + 992 字节 payload（共 1024）。`z42 apphost build <app.zpkg> [--out <path>]`（z42 写，`core/apphost.z42`）：

1. 在 stub 模板文件里按 MAGIC 定位占位区；
2. 把 payload 覆写为 app 的 **zpkg 路径** + NUL（路径形态见下两种模式）；
3. 写出到目标位置、置可执行位；
4. **macOS：ad-hoc 重签名**（见下）。

**两种输出模式**（占位符总是相对路径，运行时按 **exe 自身目录**（`current_exe`）解析 ⇒ 整个 `<exe + app.zpkg>` 可整体搬迁）：

- **默认**：exe 放在 app.zpkg **同目录**、内嵌 **basename** → 解析 `<exe_dir>/<app>.zpkg`，**与 exe 文件名无关 ⇒ 改名安全**。
- **`--out <path>`**：exe 写到任意 `<path>`、内嵌 app.zpkg **相对 `<path>` 所在目录**的路径。典型用法：`apphost build artifacts/xtask/xtask.zpkg --out xtask` 在仓库根产出 `./xtask`，内嵌 `artifacts/xtask/xtask.zpkg` → `./xtask build package` 直跑 xtask，免敲 `z42 artifacts/xtask/xtask.zpkg -- …`。一键重生脚本 `scripts/build-xtask.sh`（编 stub + launcher.zpkg + xtask.zpkg → patch）；`./xtask` 原生 + 平台相关 + 已 gitignore，**重生不提交**。

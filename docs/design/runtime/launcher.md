...
统一 trampoline → apphost 后，**`z42`（launcher）与 per-app apphost 共用同一个 stub、同一套 `probe_app_runtime`**。stub 不捆绑 VM/libs（framework-dependent），按**三档**定位 **`z42vm`**（同目录推出 `libs/`，设 `Z42_LIBS`；**不需 `launcher.zpkg`**）：

```
① 自定义覆写   $Z42_PORTABLE_VM                     显式指向 z42vm（文件或其目录）；CI/镜像/特殊部署绕过约定
② exe 近邻     （锚定 current_exe 所在目录，非 CWD）
   a. {exe_dir}/bin/z42vm                            并列 / SDK 内置 —— launcher z42（SDK 根，bin/z42vm 同级）、self-contained app 命中
   b. 逐级上行 <d>/.z42/bin/z42vm                     项目 venv —— xtask 这类 app（.z42 与 exe 并列或在上层）命中
③ 系统安装     $Z42_HOME/bin/z42vm（未设 $Z42_HOME → 默认 $HOME/.z42/bin/z42vm）
   都无 → 报错列已查路径，非零退出
```

**② 内 a 在 b 之前是正确性硬要求**：统一后 `z42` 也是 apphost，payload（`programs/launcher/launcher.zpkg`）永远按 exe 目录解析 ⇒ launcher.zpkg 来自 **z42 自己那个包**，与同包 `bin/z42vm` 同版本（zbc ABI 必配）。所以 launcher 必须先命中 exe 同级的 `bin/z42vm`（a），**绝不能落到项目本地（b）/ 系统（③）的异版本 vm**（否则新版 launcher.zpkg 在旧版 vm 上加载 → zbc 不兼容炸）。

**一套顺序同时服务两个角色**——靠"exe 同级有没有 `bin/z42vm`"自然分流：
- **launcher `z42`**（SDK 内置，`bin/z42vm` 同级）→ ②a 命中自己的 vm，永不串到项目本地 ✓
- **per-app `./xtask`（framework-dependent）**（exe 同级无 `bin/z42vm`）→ ②a 落空 → ②b 命中项目 `.z42`，**venv 语义（项目钉死 runtime）不变** ✓
- **per-app self-contained（捆 vm，future）** → ②a 命中捆绑 vm ✓

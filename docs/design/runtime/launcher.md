...
│   ├── z42vm
│   ├── libs/
│   ├── workloads/<wl>/         #   平台 workload（版本作用域：ABI 配套 <ver>，见 runtime-workload-distribution.md）
│   └── link.txt                # 可选：重定向到含 z42vm+libs 的本地目录
└── config.toml                 # default = "<ver>"
```

> **命令 vs workload 归属（2026-06-20 裁决）**：
> - **SDK 命令**（`build`/`test`/`fmt`/`new`…，纯工具、无 native ABI 耦合）**跟当前 SDK 走**，装在 `programs/`，**不进 `runtimes/<ver>/`**；多版本命令作用域延后（见 [launcher-command-dispatch.md](../toolchain/launcher-command-dispatch.md) `cmd-future-version-scoped`）。其中 **`z42 build`（纯编译，src→zpkg）= `programs/z42c/`**。
> - **平台 workload**（`publish`/`export` 的 ios/android/wasm 命令 + 配套 native 包如 xcframework）**保持版本作用域**于 `runtimes/<ver>/workloads/<wl>/`——native 嵌入件 ABI 必须配 runtime 版本（见 [runtime-workload-distribution.md](../toolchain/runtime-workload-distribution.md) Decision 5）。
> - `publish`（编译+裁剪+workload+打包可分发件）/ `export`（IDE 工程）语义见 [platform-export-lifecycle.md](../toolchain/platform-export-lifecycle.md)。
...
**便携解析**（launcher-at-package-root 2026-06-04；apphost 化 2026-06-20）：`z42`（apphost）在包根，payload 打包时 patch 为 `programs/launcher/launcher.zpkg`（相对路径）；z42vm probe = `{exe_dir}/bin/z42vm`（优先）→ `$Z42_HOME/bin/z42vm` → 本地 `.z42/bin/z42vm` 上行搜索。installed 与便携模式 **probe 路径相同**（exe 同级的 `bin/z42vm`），无需区分两套逻辑。核心的 `run`/`which` 在未 pin `--runtime` 时直接用这个 portable runtime（不查 `runtimes/<ver>`）。于是 `<pkg>/z42 run app.zpkg` 开箱即用，**不重复 z42vm/libs、不用 symlink**（Windows 友好）。`z42` 在根而非 `bin/`：它是包的统一入口，`bin/` 留给工具（z42c/z42vm/apphost 模板）。
...
```
1. $Z42_HOME/bin/z42vm                        显式配置覆写，最高优先
2. 本地：从 exe 目录起逐级上行，<d>/.z42/bin/z42vm（portable 式）
3. 系统：$HOME/.z42/bin/z42vm
4. 都无 → 报错列已查路径，非零退出
```

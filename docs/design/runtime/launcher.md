...
### 运行时解析：z42vm 探测顺序（统一 apphost 唯一真相，2026-06-20）

统一 trampoline → apphost 后，**`z42`（launcher）与 per-app apphost 共用同一个 stub、同一套 `probe_app_runtime`**。stub 不捆绑 VM/libs（framework-dependent），按下列顺序定位 **`z42vm`**（同目录推出 `libs/`，设 `Z42_LIBS`；**不需 `launcher.zpkg`**）：

```
0. $Z42_PORTABLE_VM                           自定义覆写：显式指向 z42vm（文件或其所在目录），最高优先
1. {exe_dir}/bin/z42vm                        colocated：exe 同级——launcher 自带 vm / self-contained app 命中
2. $Z42_HOME/bin/z42vm                        Z42_HOME 指定的安装根
3. 本地上行：从 exe 目录逐级上行找 <d>/.z42/bin/z42vm   venv 式：项目带 .z42 钉死 runtime
4. 系统：$HOME/.z42/bin/z42vm                  系统默认安装
5. 都无 → 报错列已查路径，非零退出
```

**为什么 `colocated`（步骤 1）必须在 `本地上行`（步骤 3）之前**：统一后 `z42` 也是 apphost，它的 payload（`programs/launcher/launcher.zpkg`）永远按 exe 目录解析 ⇒ launcher.zpkg 来自 **z42 自己那个包**。launcher.zpkg 与配套 `bin/z42vm` 同包同版本（zbc ABI 必配），所以 launcher 必须优先用 exe 同级的 `bin/z42vm`，**绝不能落到项目本地 / 系统的异版本 vm**（否则新版 launcher.zpkg 在旧版 vm 上加载 → zbc 不兼容炸）。

**一套顺序为何同时服务两个角色**——靠"是否自带 colocated vm"自然分流：
- **launcher `z42`**（root + `bin/z42vm` 同级）→ 步骤 1 命中自己的 vm，永不串到项目本地 ✓
- **per-app（framework-dependent）`./myapp`**（exe 同级无 `bin/z42vm`）→ 步骤 1 落空 → 步骤 3 命中项目 `.z42`，**venv 语义（项目钉死 runtime）不变** ✓
- **per-app（self-contained，捆 vm，future）** → 步骤 1 命中捆绑 vm ✓

**自定义覆写**：`$Z42_PORTABLE_VM`（已用于 standalone / 测试场景）置顶——CI、镜像、特殊部署可显式指定任意 z42vm，绕过全部约定路径。`$Z42_HOME` 仍可改安装根（步骤 2）。

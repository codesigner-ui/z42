...
- **CI job**（目标拓扑）：
  - `test (<平台>)`（原 `build-and-test`，4 OS）：vm goldens(interp) + cross-zpkg + stdlib + compiler-z42。
...
`test (<平台>)` 在 4-OS matrix 跑：`linux-x64`（ubuntu-latest）/ `linux-arm64`（ubuntu-24.04-arm）/ `macos-arm64`（macos-15）/ `windows-x64`（windows-latest）。详见 [`building/cross-platform.md`](building/cross-platform.md)。

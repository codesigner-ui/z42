...
⚠️ `./scripts/install-android-toolchain-local.sh` 在 Windows 上**会拒绝执行**（只支持 macOS/Linux 自动下载）；走 Android Studio 这条路。
...
⚠️ `./scripts/install-node-local.sh` 在 Windows 上也会**拒绝执行**（POSIX .tar.gz 路径）；走 MSI 这条路。

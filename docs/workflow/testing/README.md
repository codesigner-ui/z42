...
`./xtask test` 默认跑 6 stages（dotnet build / cargo build /
...
```bash
./xtask test     # 跳 dotnet stages，约省 30-40s
./xtask test     # 跳 build + dotnet test，约省 60-90s
./xtask test     # 自动判断当前 diff
./xtask test     # full，commit 前必走
```

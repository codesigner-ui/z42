//! Unit tests for `signal_handler` — `sigsafe` write primitives + signal
//! name table + install idempotency. The actual signal-firing path is
//! covered by `tests/signal_handler_e2e.rs` integration tests.

use super::sigsafe;
use super::signal_name;

/// Capture writes by piping into a `pipe(2)` and reading the read end.
/// This lets us assert exact byte sequences without touching real stderr.
...
fn write_dec_u32_max() {
    assert_eq!(
        capture(|fd| sigsafe::write_dec_u32(fd, u32::MAX)),
        b"4294967295"
    );
}
...
fn write_hex_u64_max() {
    assert_eq!(
        capture(|fd| sigsafe::write_hex_u64(fd, u64::MAX)),
...
fn write_str_partial_safety() {
    // Verify that splitting input across multiple internal write() calls
    // produces same output as one big write.
...
fn signal_name_unknown() {
    // Pick a signal that's never in our covered list (SIGUSR1 is fine).
...
fn install_is_idempotent() {
    ...
    // register handlers. We can't inspect the static INSTALLED atomic
    // directly from outside the module without exposing it, but the call
    // shouldn't crash even if it re-registers handlers (signal-hook-registry
    // queues all handlers and runs them in order — duplicate registration
    // just means our handler runs twice, harmless but wasteful).
    super::install();
    super::install();
    // Reaching here = no panic + no abort = pass.
}

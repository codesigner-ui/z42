...
| 文件 | 职责 |
|------|------|
| `ISymbol.cs` | **review.md F2.2 Phase 1**（add-isymbol-base-phase1, 2026-06-03）— Roslyn `ISymbol` 平行根接口 / `Name` / `Kind` / `DeclarationSpan` / `Visibility` |
| `SymbolKind.cs` | 同上 — 枚举（Method / Field / Class / Interface / Local / Parameter / ...） |
| `IMemberSymbol.cs` | extends `ISymbol`；保留原 `Span` / `ContainingType: Z42Type?` 字段，`DeclarationSpan => Span` 桥接默认实现 |
| `IMethodSymbol.cs` | 方法接口 + `MethodSymbol` sealed class 实现；含 `Kind => Method` / `Signature` / `Modifiers` / `Decl?` / `TestAttributes?` 字段 |
| `IFieldSymbol.cs` | 字段接口 + `FieldSymbol` sealed class 实现；含 `Kind => Field` / `Type` / `IsStatic` / `IsEvent` / `Decl?` 字段 |

...
///
/// spec split-bind-call (2026-05-12, closes D-12): single 440-line BindCall
/// split into a thin dispatcher + 9 receiver-shape-specific helpers. Zero
/// behavior change — preserves every diagnostic / bound-expr shape produced
/// by the original monolith. Splits matched the natural if-chain boundaries
/// in the source so each branch reads as a self-contained "bind one shape"
/// routine, callable in isolation by future call-related refactors.
public sealed partial class TypeChecker
{
    ...
    private BoundExpr BindCall(CallExpr call, TypeEnv env)
    {
        ...
        // Static class method: ClassName.Method(args). May fall through to
        // member-call binding when the class is local-not-imported and the
        // member name isn't a static method (i.e. instance call on a class
        // ident — handled by BindMemberCall via type-of-receiver dispatch).
        if (call.Callee is MemberExpr
                { Target: IdentExpr { Name: var clsName }, Member: var staticMember }
            && _symbols.Classes.TryGetValue(clsName, out var staticCt))
        {
            var staticResult = TryBindStaticClassCall(clsName, staticMember, staticCt, call, env);
            if (staticResult is not null) return staticResult;
        }

        if (call.Callee is MemberExpr mCallee)
            return BindMemberCall(mCallee, call, env);

        return BindFreeFunctionCall(call, env);
    }

    // ── Static class method: ClassName.Method(args) ───────────────────────────

    /// Returns null when the class exists locally but no static method matched
    /// AND the class isn't imported — caller falls through to member-call
    /// dispatch (e.g. `ct.Method(args)` where Method is an instance method).
    private BoundExpr? TryBindStaticClassCall(
        string clsName, string staticMember, Z42ClassType staticCt,
        CallExpr call, TypeEnv env)
    {
    ...
    // ── Member call dispatcher ────────────────────────────────────────────────

    private BoundExpr BindMemberCall(MemberExpr mCallee, CallExpr call, TypeEnv env)
    {
        ...
        {
            return BindMemberCallOnUnknownTarget(mCallee, tgtName, call, env);
        }
        ...
        if (recvExpr.Type is Z42InstantiatedType inst)
            return BindMemberCallOnInstantiated(recvExpr, inst, mCallee, call, argBound, env);

        if (recvExpr.Type is Z42ClassType ct)
            return BindMemberCallOnClass(recvExpr, ct, mCallee, call, argBound, env);

        if (recvExpr.Type is Z42InterfaceType ifaceType)
            return BindMemberCallOnInterface(recvExpr, ifaceType, mCallee, call, argBound, env);
        ...
        if (recvExpr.Type is Z42GenericParamType gp)
            return BindMemberCallOnGenericParam(recvExpr, gp, mCallee, call, argBound, env);

        return BindMemberCallOnPrimitive(recvExpr, mCallee, call, argBound, env);
    }

    // ── Member call: unknown ident target (stdlib / primitive keyword) ────────
    ...
    // ── Member call: Z42InstantiatedType (L3-G4a) ─────────────────────────────

    private BoundExpr BindMemberCallOnInstantiated(
        BoundExpr recvExpr, Z42InstantiatedType inst,
        MemberExpr mCallee, CallExpr call,
        List<BoundExpr> argBound, TypeEnv env)
    {
    ...
    // ── Member call: Z42ClassType (instance method on user / imported class) ──

    private BoundExpr BindMemberCallOnClass(
        BoundExpr recvExpr, Z42ClassType ct,
        MemberExpr mCallee, CallExpr call,
        List<BoundExpr> argBound, TypeEnv env)
    {
        var instArityKey = $"{mCallee.Member}${argBound.Count}";
    ...
    // ── Member call: Z42InterfaceType ─────────────────────────────────────────

    private BoundExpr BindMemberCallOnInterface(
        BoundExpr recvExpr, Z42InterfaceType ifaceType,
        MemberExpr mCallee, CallExpr call,
        List<BoundExpr> argBound, TypeEnv env)
    {
    ...
    // ── Member call: Z42GenericParamType (L3-G2 / G2.5) ───────────────────────

    private BoundExpr BindMemberCallOnGenericParam(
        BoundExpr recvExpr, Z42GenericParamType gp,
        MemberExpr mCallee, CallExpr call,
        List<BoundExpr> argBound, TypeEnv env)
    {
        ...
        {
            return BindGenericParamBaseClassMethod(
                recvExpr, bc, bcMtSym, mCallee, call, argBound, env);
        }
        foreach (var iface in bundle.Interfaces)
        {
            var result = TryBindGenericParamInterfaceMethod(
                recvExpr, iface, gp, mCallee, call, argBound, env);
    ...
    /// Generic-param base-class method dispatch (L3-G2.5 walks bundle.BaseClass).
    private BoundExpr BindGenericParamBaseClassMethod(
        BoundExpr recvExpr, Z42ClassType bc, IMethodSymbol bcMtSym,
        MemberExpr mCallee, CallExpr call,
        List<BoundExpr> argBound, TypeEnv env)
    {
    ...
    /// Generic-param interface-constraint method dispatch (L3-G2 + L3 static
    /// abstract). Returns null when neither instance nor static-abstract
    /// member matches on this interface — caller continues to next iface.
    private BoundExpr? TryBindGenericParamInterfaceMethod(
        BoundExpr recvExpr, Z42InterfaceType iface, Z42GenericParamType gp,
        MemberExpr mCallee, CallExpr call,
        List<BoundExpr> argBound, TypeEnv env)
    {
    ...
    // ── Member call: primitive / unknown / L3 primitive-as-struct fallback ────

    private BoundExpr BindMemberCallOnPrimitive(
        BoundExpr recvExpr, MemberExpr mCallee, CallExpr call,
        List<BoundExpr> argBound, TypeEnv env)
    {
    ...
    private BoundExpr BindFreeFunctionCall(CallExpr call, TypeEnv env)
    {
        ...
        if (call.Callee is IdentExpr funcId)
            return BindFreeIdentCall(funcId, call, freeArgs, env);
    ...
    /// Identifier callee — dispatcher among top-level fn / fn-valued var /
    /// generic-param-with-FuncSig var / unknown-fn diagnostic.
    private BoundExpr BindFreeIdentCall(
        IdentExpr funcId, CallExpr call, List<BoundExpr> freeArgs, TypeEnv env)
    {
        ...
        if (funcType is Z42FuncType ft)
            return BindFreeTopLevelCall(funcId, ft, resolvedFuncName, call, freeArgs, env);

        var varType = env.LookupVar(funcId.Name);
        if (varType is Z42FuncType vft)
            return BindFreeFuncValuedVarCall(funcId, vft, call, freeArgs, env);
    ...
    /// Top-level / imported function reached via env.LookupFunc.
    private BoundExpr BindFreeTopLevelCall(
        IdentExpr funcId, Z42FuncType ft, string resolvedFuncName,
        CallExpr call, List<BoundExpr> freeArgs, TypeEnv env)
    {
    ...
    /// Function-valued local variable (impl-lambda-l2 / impl-closure-l3-monomorphize).
    /// `var f = x => x + 1; f(5);` or `var f = Helper; f();`.
    /// See docs/design/language/closure.md §6.
    private BoundExpr BindFreeFuncValuedVarCall(
        IdentExpr funcId, Z42FuncType vft, CallExpr call,
        List<BoundExpr> freeArgs, TypeEnv env)
    {

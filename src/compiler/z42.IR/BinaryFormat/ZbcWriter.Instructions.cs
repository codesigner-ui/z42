...
public static partial class ZbcWriter
{
    ...
    {
        // Phase 3 S3 (tokenize-ir-and-zbc-bump, 2026-05-09): tokenizable IR
        // fields write u32 token (intra-module direct id, or ImportBase+idx
        // for cross-zpkg). Untokenized fields (Static*.Field, BuiltinInstr.Name,
        // VCall.Method, Field*.FieldName, CallNative*) continue using
        // pool.Idx(...) str_idx encoding.
        switch (instr)
        {
            ...
            case CallInstr i:
                ...
                w.Write(allocator.ResolveMethod(i.Func).Value);
                WriteArgs(w, i.Args);
                break;
            ...
            case MkClosInstr i:
                ...
                w.Write(allocator.ResolveMethod(i.FuncName).Value);
            ...
            case ObjNewInstr i:
                ...
                w.Write(allocator.ResolveType(i.ClassName).Value);
                w.Write(allocator.ResolveMethod(i.CtorName).Value);
                WriteArgs(w, i.Args);
            ...
            case IsInstanceInstr i:
                ...
                WriteReg(w, i.Obj); w.Write(allocator.ResolveType(i.ClassName).Value);
                break;
            case AsCastInstr i:

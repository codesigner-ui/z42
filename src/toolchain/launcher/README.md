...
| `core/apphost.z42` | **z42** | apphost stub-patch 库（apphost-as-config 2026-06-17）：拷贝 apphost stub 模板 + patch 内嵌占位符 + macOS ad-hoc 重签名。`Produce(app, outPath)` 由 `_cmdExportDesktop`（`z42 export desktop`）调用——**无独立 `z42 apphost` 命令**。 |
...
| `src/apphost.rs` | Rust | 每-app 原生 apphost stub **模板**：内嵌占位符（`z42 export desktop` patch 成 app zpkg 名）→ 运行时本地优先解析 → 起 app。占位符须 **volatile 读**（防 release const-fold）。 |
...
`z42 export desktop <project.z42.toml>`（读 `[platform.desktop].publish_dir`）→ 产出 per-app 原生可执行文件 apphost（机制 / 本地优先解析 / 签名详见 [`docs/design/runtime/launcher.md`](../../../docs/design/runtime/launcher.md) 的 apphost 段）。apphost-as-config：apphost 是 desktop 平台的发布产物，不是独立命令。

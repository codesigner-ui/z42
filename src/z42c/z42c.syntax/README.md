...
测试：`tests/{lexer 17, parser 19, stmt 15, decl 24}`（共 75），经 `xtask test compiler-z42`。
待移植（incr 6d+）：真实 Visitor 基类 + TypeExpr AST（替代 Dump/类型文本字符串，架构性）；lambda；byte-identical 对账；转义 `\0`/`\uXXXX` 解码。

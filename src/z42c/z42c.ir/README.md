...
| `src/IrInstr.z42` | IrInstr 基类 + virtual Dump() + 子类（class-per-instruction）：ConstI32/I64/F64/Bool/Str/Null + Copy + Add/Sub/Mul/Div/Rem + Call/VCall/FieldGet/FieldSet（CG-1C）+ ObjNew/ArrayGet/ArraySet/IsInstance/AsCast（CG-1D）。按增量加比较/逻辑/一元等 |
...
## 增量进度
CG-1A IR 内存模型 + 最小指令集（const/copy/算术 + ret）✅ / CG-1B 控制流（Br/BrCond + 多基本块）✅ / CG-1C 调用·字段（Call/VCall/FieldGet/FieldSet）✅ / CG-1D new·数组·is·as（ObjNew/ArrayGet/ArraySet/IsInstance/AsCast）✅。后续：CG-1E 比较·一元·逻辑·拼接·三目 / CG-2 泛型。byte-identical .zbc（ZbcWriter）独立 change。

...
| `src/IrInstr.z42` | IrInstr 基类 + virtual Dump() + 子类（class-per-instruction）：ConstI32/I64/F64/Bool/Str/Null + Copy + Add/Sub/Mul/Div/Rem + Call/VCall/FieldGet/FieldSet（CG-1C）。按增量加比较/逻辑/对象等 |
...
## 增量进度
CG-1A IR 内存模型 + 最小指令集（const/copy/算术 + ret）✅ / CG-1B 控制流（Br/BrCond + 多基本块）✅ / CG-1C 调用·字段（Call/VCall/FieldGet/FieldSet）✅。后续：CG-1D new·cast·is·数组 / CG-1E 比较·逻辑·拼接。byte-identical .zbc（ZbcWriter）独立 change。

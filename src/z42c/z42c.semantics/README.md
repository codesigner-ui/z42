...
| `src/FunctionEmitter.z42` | **codegen 半**（CG-1A）：每函数 Bound → IR lowering。集中 if-is EmitExpr→TypedReg / EmitStmt；寄存器分配 + 块管理；Z42Type→IrType 映射（叶子 z42c.ir 不引用 Z42Type，映射在此） |

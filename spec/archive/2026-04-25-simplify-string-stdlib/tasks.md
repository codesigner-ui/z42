...
> 状态：🟢 已完成 | 创建：2026-04-23 | 实施：2026-04-25

## 进度概览
- [x] 阶段 1: VM 侧 builtin 调整
- [x] 阶段 2: 标准库脚本重写
- [x] 阶段 3: 测试与验证
- [x] 阶段 4: 文档与归档
...
- [x] 1.1 在 `src/runtime/src/corelib/string.rs` 新增 `builtin_str_char_at` / `builtin_str_from_chars`
- [x] 1.2 新建 `src/runtime/src/corelib/char.rs`（或就地），实现 `builtin_char_is_whitespace` / `builtin_char_to_lower` / `builtin_char_to_upper`
- [x] 1.3 `src/runtime/src/corelib/mod.rs` 的 dispatch 注册表中：
...
- [x] 1.4 从 `src/runtime/src/corelib/string.rs` 删除对应的 13 条 `builtin_str_*` 函数 + 单元测试
- [x] 1.5 `cargo build` + `cargo test` 确认 Rust 侧编译通过

## 阶段 2: 标准库脚本重写

- [x] 2.1 扩展 `src/libraries/z42.core/src/Char.z42`：新增 `IsWhiteSpace` / `ToLower` / `ToUpper` 三个 extern
- [x] 2.2 重写 `src/libraries/z42.core/src/String.z42`：
...
- [x] 2.3 运行 `./scripts/build-stdlib.sh` 重编译 zpkg 产物
- [x] 2.4 `Replace` 空串 `old=""` 边界：`throw new Exception("Replace: oldValue cannot be empty")`（对齐 C# ArgumentException 语义，用 z42.core 已有的 Exception）
- [x] 2.5 `Replace` 采用两遍扫描 + `char[]` 分配（不引入 List<T>，保持 z42.core 零依赖）

## 阶段 3: 测试与验证

- [x] 3.1 新建 `src/runtime/tests/golden/run/88_string_script/` 端到端 golden，覆盖 spec 中每个 scenario
- [x] 3.2 检查现有 golden（`examples/async.z42` 等使用 string 方法的）仍然通过
- [x] 3.3 `src/compiler/z42.Tests/` 若有 string builtin 专项测试，调整为脚本方法形态
- [x] 3.4 `dotnet build src/compiler/z42.slnx` 无错误
- [x] 3.5 `cargo build --manifest-path src/runtime/Cargo.toml` 无错误
- [x] 3.6 `dotnet test src/compiler/z42.Tests/z42.Tests.csproj` 100% 通过
- [x] 3.7 `./scripts/test-vm.sh` 100% 通过（interp + jit 两模式）
- [x] 3.8 spec scenarios 逐条覆盖确认（制作 Scenario → 测试位置对照表）
...
- [x] 4.5 归档：`spec/changes/simplify-string-stdlib/` → `spec/archive/2026-04-23-simplify-string-stdlib/`

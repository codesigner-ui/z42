...
[interop.md](../../../docs/design/interop.md) 解决的是 "native 代码 → 注册类型/方法进 z42"（类比 CPython C 扩展）；当前缺的是另一面 —— **宿主 app → 启动 VM → 加载 .zbc → 调用入口 → 关闭**（类比 CoreCLR `coreclrhost.h` / JNI `JavaVM` / Lua `lua_State`）。

没有这层 API：

- iOS / Android app 不能内嵌 z42（[cross-platform.md](../../../docs/design/cross-platform.md) 仅解决 VM 怎么编译，没解决怎么被宿主调用）
- [cross-platform-testing.md](../../../docs/design/cross-platform-testing.md) 的 test-runner library 必须自己直连 VM internals，与平台 facade 共用一份调用路径困难
...
- **新建 [docs/design/embedding.md](../../../docs/design/embedding.md)**（已 DRAFT）— 三层 ABI 设计、单实例语义、生命周期、错误码、Hello World 示例、Deferred 清单
...
- **修改 [docs/design/vm-architecture.md](../../../docs/design/vm-architecture.md)** — H1 实施时追加"Embedding 入口"小节

## Scope

| 文件路径 | 变更类型 | 阶段 |
|---------|---------|------|
| `docs/design/embedding.md` | NEW | H0 (已完成) |
| `spec/changes/add-embedding-api/{proposal,design,tasks}.md` | NEW | H0 |
| `spec/changes/add-embedding-api/specs/embedding-host-api/spec.md` | NEW | H0 |
...
| `docs/design/vm-architecture.md` | MODIFY | H1 |
...
- [docs/design/interop.md](../../../docs/design/interop.md) — Tier 1/2/3 分层哲学
- [docs/design/cross-platform.md](../../../docs/design/cross-platform.md) — feature flag 与 ExecMode 关系

## Out of Scope

- **多 VM 实例 / ALC-like context**：v0.1 单实例；进 [embedding.md §12 Deferred](../../../docs/design/embedding.md)
- **Hot reload**：依赖多实例
...
- [ ] **Q1**：`Z42Value` 字符串类型是否在 v0.1 即支持？hello-world 入口签名 `(string) -> int`，需要至少能传一个 string 进去。
  - 倾向：**是**。沿用 [interop.md §6.3 pinned](../../../docs/design/interop.md) 模型；v0.1 host 端只支持"宿主分配的 UTF-8 buffer + 长度"形式（不直接构造 z42 GC string）。
- [ ] **Q2**：`Z42HostStatus` 与 [error-codes.md](../../../docs/design/error-codes.md) 编号空间是否冲突？

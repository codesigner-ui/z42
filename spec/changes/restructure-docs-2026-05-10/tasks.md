...
> 状态：🟡 进行中 | 创建：2026-05-10 | 类型：docs / refactor（最小化模式）
...
- [ ] Phase 1: 归档 review.md + 外部依赖更新（workflow.md / memory）
- [ ] Phase 1.5: 把 `spec/` 整体挪到 `docs/spec/`（保留子结构）+ 全仓批量更新引用
...
- [x] 1.1 `docs/review.md` → `spec/archive/2026-05-10-docs-review/review.md`，加 `## Status` 头部声明"快照，已归档"
...
- [ ] 1.5.1 `git mv spec docs/spec`（一次性整目录迁移）
- [ ] 1.5.2 全仓 grep 更新引用：`spec/changes/` → `docs/spec/changes/`、`spec/archive/` → `docs/spec/archive/`、`spec/` → `docs/spec/`（受影响：`.claude/rules/workflow.md`、`.claude/CLAUDE.md`、`docs/README.md`、`docs/dev.md`、`docs/roadmap.md`、`docs/version.md`、`docs/features.md`、`docs/design/*.md` 内引用、各 archive 内 proposal.md 互引、`scripts/*.sh`（如有）
- [ ] 1.5.3 验证：grep 全仓不再出现裸 `spec/changes` / `spec/archive` 路径（除历史 commit message 外）
- [ ] 1.5.4 commit + push（备注：spec 目录内文件已被 git 自动追踪 rename，diff 应主要是路径与引用变更）
...
- [ ] 4.9 `language/interop.md` L1 interim 段拆分到 `spec/archive/2026-05-10-l1-interim-interop-snapshot/`，主文档仅留 L2+ 设计
- [ ] 4.10 commit + push

...
- ❌ 批量授权模式下自行决定 — 属于阶段 6.5 中断条件 4（决策点未覆盖）/ 8（架构性发现）

### 与其他规则的关系

- 与 **修复必须从根因出发**（本文件）配合：根因若是"设计已偏移"，按本规则停下
- 与 **规范冲突检测**（CLAUDE.md）配合：spec 与代码现状冲突时，本规则适用
...
**延后特性按"识别时机"分两类，记录位置不同：**

| 时机 | 性质 | 记录位置 |
|------|------|---------|
| **设计期延后**（spec proposal/design 起草期）| 主动决定"本次不做、形态待评估" | 对应 `docs/design/<feature>.md` 的 "Deferred / Future Work" 段 |
| **实施期延后**（spec 实施 / fix / refactor 期）| 发现 Scope 外新问题、新限制、新缺陷，经 User 裁决"暂不做" | `docs/deferred.md` |

**判断标准：**

- **设计期延后** = 在写第一行实现代码前就决策的"暂不引入"；它是当前特性"完整设计图景"的一部分，与正向规范并列阅读最自然。例：lang spec 中明确划入 Out of Scope 的子特性、design.md 中讨论后选择不引入的形态变体。
- **实施期延后** = 实施过程中浮现、需要 User 裁决的"暂不做"债务；它通常是 Scope 越界发现的衍生问题，需要跨 spec 集中追溯。例：实施期发现某个前置依赖未就绪、规范冲突需要独立 spec 处理。

**为什么分两处：**

- design doc 里的延后段落是"此特性的边界与未来空间"，给读者展示当前形态的简化轨迹；混入 deferred.md 会让特定特性的延后散落到全局债务簿，干扰其他特性的 backlog 追溯
- deferred.md 是跨 spec 实施债务簿，集中记录"实施中产生的债"，便于定期回顾清理；它的作用是债务簿而非设计图

**禁止反例：**

- ❌ proposal/design 起草期把延后子特性写进 `docs/deferred.md`（污染债务簿性质）
- ❌ 实施期发现的限制只在 design doc 里写"未来再看"（缺乏跨 spec 追溯能力）
- ❌ 同一项在两处都登记（重复记录、决策权威分散）
- ❌ 把"未来某个 phase 可能引入"的展望式描述写进 deferred.md（那属于 roadmap 或 design doc）

**与 feedback_problem_first_then_defer 的关系：**

- 该规则覆盖"实施期发现新问题"的处理流程（停 → 汇报 → User 裁决 → 进 deferred.md）
- 本规则进一步明确"如果是设计期就决策的延后，应该在 design doc 内就近记录"
- 两条规则共同保证：设计期决策与实施期债务在不同载体留痕，不交叉

...
资源加载循环（`Directory.EnumerateFiles` / `HashSet` 迭代 + `TryAdd` first-wins 等）的确定性约束见 [common-pitfalls.md §1](common-pitfalls.md#1-资源加载顺序必须显式排序2026-05-17-强化)。该规则跨语言适用（C# / Rust / bash 都涉及），统一在 common-pitfalls.md 沉淀。

...
- [reference_release_vm_jit_miscompiles_default_params.md](reference_release_vm_jit_miscompiles_default_params.md) — release z42vm JIT 误编 default_params/delegate/event(空输出);debug vm JIT 正常;同 .zbc interp 全过。vm-jit 用 debug vm 故绿;compile-once 消费 toolchain(只装 release vm)jit 因此挂——出路:toolchain 装 debug vm 或修 release-jit。跨平台复现
